...
# --- Два тумблера под ответственность владельца. Дефолты БЕЗОПАСНЫЕ. --------
# Раскомментируй только осознанно и только по нише клиента.
#
# strict_knowledge: true (дефолт) — Аня говорит ТОЛЬКО из knowledge.md;
#   обещание вне базы (скидка/гарантия/«перезвоню») подавляется и уходит тебе.
#   false — свободнее: такое обещание доходит до лида, но карточку ты всё равно
#   получаешь. Юр-услуги/медицина обычно хотят true; SMM часто — false.
#   В ОБОИХ режимах выдуманные ЦЕНЫ/СРОКИ и запрещённые термины подавляются.
# strict_knowledge: false
#
# honesty_mode: honest (дефолт) — на прямой «ты бот?» Аня раскрывается честно
#   и предлагает владельца. Второе значение снимает эту гарантию; оно намеренно
#   длинное, чтобы не проставилось по опечатке, и пишется в лог при каждой
#   загрузке. В ряде юрисдикций раскрытие обязательно по закону — решение и
