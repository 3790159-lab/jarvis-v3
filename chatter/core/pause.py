...
def human_holds_dialog(contact_row: dict | None) -> bool:
    """Открыт ли ЭПИЗОД ПЕРЕХВАТА: диалог прямо сейчас ведёт человек.

    Не синоним `is_muted`. Пауза бывает по разным причинам (команда владельца,
    снуз из карточки, глобальный рубильник), и молчать бот обязан при любой; а
    вот «человек уже отвечает этому лиду» — это отдельный факт, и чинится он
    по-другому: не `/resume` по таймеру, а тем, что человек договорил.

    Нужен там, где ответ бота уже сочинён и вот-вот уйдёт: между решением
    ответить и отправкой проходит wall-clock время (пауза чтения+печати), и за
    это время владелец мог нажать «отправить» в панели. Спека 2026-08-25 §7.

    Источник паузы сравнивается со строкой `'human_takeover'` — тем же
    значением, которое пишет `Store.begin_takeover` (и которое перечислено в
    `db.ROW_MUTE_SOURCES`). Импортировать его отсюда нельзя: этот модуль
    чистый — ноль SQLite, ноль Telethon, — и ровно поэтому решение о молчании
    проверяется без аккаунта и без базы."""
    if not contact_row or not contact_row.get("paused"):
        return False
    return (contact_row.get("pause_source") or "") == "human_takeover"
