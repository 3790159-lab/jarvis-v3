...
def classifier_system_prompt(playbook: str, language: str,
    ...
    if track_obligations:
        ...
        obl_section = (
            ...
            "recalc=названа сумма/зафиксирован запрос. owner_write ты только "
            "СОЗДАЁШЬ (open), когда обещано «керівниця напише»; закрывать или "
            "отменять owner_write НЕЛЬЗЯ — его статус ведёт ПРОГРАММА по факту "
            "доставленной карточки. Обязательство БЕЗ изменений можно не "
