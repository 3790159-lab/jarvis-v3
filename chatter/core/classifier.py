...
def classifier_stable_prefix(playbook: str, language: str,
    ...
    # Ленивый импорт намеренно: `classifier` держит РОВНО один core-импорт
    # (`window`) и импортируется в офлайне и в тестах пачками. `escalation`
    # тянет conversation/guardrails/disclosure/obligations — тащить этот хвост
    # в шапку ради одной вырезалки значило бы утяжелить самый лёгкий модуль
    # ядра. Кольца импортов сегодня нет, но ленивый импорт делает его
    # невозможным и завтра.
    from chatter.core.escalation import strip_keyword_section

    profile_chars = profile_budget_tokens * _CHARS_PER_TOKEN * 2 // 3
    return (_role_block()
            # Секция ключевых слов вырезается ЗДЕСЬ, в сборщике, а не у
            # вызывающего: сторож порога кэша (§2.2) зовёт эту функцию напрямую,
            # чтобы померить префикс. Вырежи мы выше по стеку — сторож мерил бы
            # текст, которого модель не видит, и врал бы в безопасную сторону.
            + f"=== ПЛЕЙБУК ВОРОНКИ ===\n{strip_keyword_section(playbook)}\n\n"
            + _TAIL_POINTER + "\n\n"
...
def classifier_system_prompt(playbook: str, language: str,
    ...
    Единственная правка спеки срезки: вырез секции ключевых слов. Он обязан
    приехать и сюда, иначе флаг кэша менял бы не только тариф, но и то, что
    модель читает, — а ветка отката должна отличаться от рабочей ровно
    раскладкой блоков, ничем больше."""
    from chatter.core.escalation import strip_keyword_section
    ...
    return (
        ...
        "запрос вне плейбука); (2) на какой стадии воронки диалог; (3) обнови "
        "профиль клиента.\n\n"
        # Тот же вырез, что и в кэшируемом сборщике выше. Два пути (кэш вкл/выкл)
        # обязаны отдавать модели ОДИН И ТОТ ЖЕ плейбук: разойдись они, флаг
        # кэша менял бы не только тариф, но и то, что модель читает.
        f"=== ПЛЕЙБУК ВОРОНКИ ===\n{strip_keyword_section(playbook)}\n\n"
