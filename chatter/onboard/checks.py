# -*- coding: utf-8 -*-
"""T4 — автоприёмка онбординга: C1–C16 по готовому каталогу клиента.
...
уже собранный `report.json` и `brief.json` того же прогона, выход — ровно 16
...
Сеть — ровно одна и ТОЛЬКО инъекцией: C16 меряет префикс через
...
from chatter.core.prefix_budget import DEFAULT_BASELINES_PATH, check_client_prefixes
...
CHECK_IDS: tuple[str, ...] = tuple(f"C{i}" for i in range(1, 17))
...
def _c16(ctx: _Ctx) -> CheckResult:
    ...
    verdicts = check_client_prefixes(ctx.config, counter=ctx.token_counter)
    if not verdicts:
        return _blocked("C16", "сторож префикса не вернул ни одного вердикта")
...
def run_checks(client_dir, report_document: dict, *, slug: str,
    ...
    """Ровно 16 вердиктов по каталогу клиента, всегда и в порядке C1…C16.

    Каталог только ЧИТАЕТСЯ: `--check` ходит в том числе по боевому
    `chatter/clients/<slug>`, где живут деньги клиента.

    `token_counter` — единственная дверь наружу (C16, спека Хайку §9):
    `count_tokens(model, system_text) -> int`. Не задан — C16 честно говорит,
    что замер не выполнен, и прогон остаётся полностью офлайновым.
    """
    try:
