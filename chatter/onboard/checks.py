# -*- coding: utf-8 -*-
"""T4 — автоприёмка онбординга: C1–C17 по готовому каталогу клиента.
...
C16:   `docs/superpowers/specs/2026-08-19-classifier-on-haiku.md` §9
       (размер префикса brain против ЗАПИСАННОГО эталона).
План:  `docs/superpowers/plans/2026-08-17-onboard-pipeline-plan.md`.

ГРАНИЦА МОДУЛЯ. Ноль LLM, ноль записи на диск: вход — каталог из пяти файлов,
уже собранный `report.json` и `brief.json` того же прогона, выход — ровно 17
вердиктов. Каталог читается и НЕ правится ни при каких условиях: `--check`
ходит в том числе по боевому `chatter/clients/<slug>` (спека §6, калибровка), а
проверка, которая чинит то, что проверяет, ничего не доказывает.

Сеть — ровно одна и ТОЛЬКО инъекцией: C16 и C17 меряют префиксы через
`token_counter`, который передаёт вызывающий (`chatter/onboard/__main__.py`,
если в окружении есть ключ). По умолчанию счётчика НЕТ, и тогда C16 — флаг с
текстом «ЗАМЕР НЕ ВЫПОЛНЕН». Так офлайн-прогон остаётся офлайн-прогоном, а
молчания не возникает: строка про невыполненный замер стоит в отчёте.

ТРИ ПРАВИЛА, которым подчинён каждый кусок ниже.

1. **Проверок ВСЕГДА шестнадцать.** Проверка, которая молча исчезла из списка,
...
* C4 и C10 опираются на `report.json`. Битый/непереданный отчёт делает
  `blocked` РОВНО эти две проверки, а не все прочие: остальные читают файлы и
  доказывают ровно столько же, сколько доказали бы с отчётом.
...
* C16 — ФЛАГ, а не красное, и это требование §9.2 спеки: рост префикса бывает
  законным (клиент правит свой плейбук), а нового клиента без записанного
  эталона нельзя не пустить в прод по этой причине. Громко — да; отказ — нет.
"""
...
from chatter.core.prefix_budget import (
    BRAIN_DRIFT, CACHE_THRESHOLD, DEFAULT_BASELINES_PATH, check_client_prefixes)
...
CHECK_IDS: tuple[str, ...] = tuple(f"C{i}" for i in range(1, 18))
...
#
# C16 — флаг по прямому требованию §9.2 спеки Хайку: «ГРОМКО, но не отказ».
# Отличие от порога кэша (§2.3, красное): там дефект однозначен — короткий
# префикс кэш ВЫКЛЮЧАЕТ, — а здесь выросший префикс может быть законным ростом
# плейбука. Флаг ставит строку в отчёт, красное остановило бы подключение.
# C17 в этот набор НЕ входит намеренно (§2.3 «это не флаг, а красное»):
# клиент с тонким плейбуком не должен доезжать до подключения. C16 — флаг, и
# это разные требования спеки: рост префикса бывает законным, выключенный кэш —
# никогда.
...
class _Ctx:
    ...
    def __init__(self, client_dir, report_document, *, slug: str, token_counter=None):
        self.dir = Path(client_dir)
        self.slug = slug
        # Счётчик токенов для C16 — ИНЪЕКЦИЯ, и None здесь легален. Ноль сети
        # по умолчанию: `--check` гоняют в том числе там, где ключа нет вовсе,
        # и требовать его значило бы сделать всю приёмку недоступной ради
        # одного флага. Отсутствие счётчика не молчит — C16 скажет об этом.
...
# «відповідає протягом трьох годин» неотличимы для всех прежних
...
# ─────────────────────────────────────────────────────────────────────────────
# C16 — префикс brain против ЗАПИСАННОГО эталона (спека Хайку §9)
# ─────────────────────────────────────────────────────────────────────────────
#
# Зачем в онбординге. Это единственное место, где дрейф ловится ДО клиента, а
# не после — иначе о нём узнают из счёта, как узнали 19.08: цена вызова brain
# выросла ×1.63 за месяц, полоса тарифа съехала 790 → 653 диалога, и заметили
# это случайно, при пересчёте.
#
# Проверка НЕ считает ничего сама: и замер, и сравнение с эталоном делает
# `chatter.core.prefix_budget` — ТОТ ЖЕ код, который стоит на старте раннера.
# Собственная копия правила зеленеет ровно тогда, когда разъезжается с
# рантаймом, а разъезжается она молча (правило 3 в шапке модуля).

def _prefix_verdicts(ctx: _Ctx):
    """Вердикты сторожа префиксов — ОДИН замер на C16 и C17.

    Строки отчёта две (§2.3 требует порог отдельной строкой: у дрейфа и у
    порога разные причины, разные лекарства и разная тяжесть), а сеть одна.
    Два независимых вызова означали бы два `count_tokens` в приёмке и —
    важнее — возможность получить в одном прогоне два РАЗНЫХ замера одного
    префикса, если между ними что-то поменялось.
    """
    if getattr(ctx, "_prefix_cache", None) is None:
        ctx._prefix_cache = check_client_prefixes(
            ctx.config, counter=ctx.token_counter)
    return ctx._prefix_cache


def _verdict_of(ctx: _Ctx, check: str):
    return next((v for v in _prefix_verdicts(ctx) if v.check == check), None)


def _c16(ctx: _Ctx) -> CheckResult:
    if ctx.token_counter is None:
        # Флаг, а не `blocked`: прогон СОСТОЯЛСЯ, просто одна его строка не
        # мерилась. Текст начинается с «ЗАМЕР НЕ ВЫПОЛНЕН», чтобы в отчёте это
        # читалось как отсутствие данных, а не как «префикс в порядке».
        return _red("C16", (
    ...
    if ctx.config is None:
        return _blocked("C16", (
            f"клиент не поднимается ({ctx.config_error}) — системный промпт "
            f"собрать не из чего; сперва C1"), file="settings.yaml")

    v = _verdict_of(ctx, BRAIN_DRIFT)
    if v is None:
        return _blocked("C16", "сторож префикса не вернул вердикта о дрейфе brain")
    verdicts = [v]
    # Файл и строку НЕ называем намеренно, вопреки правилу 2: префикс это сумма
    # persona + knowledge + playbook + examples, и указать пальцем на один файл
    # значило бы соврать про виновника. Адрес правки называет сам текст
    # вердикта — он всегда содержит слаг, оба числа и путь файла эталонов.
    message = " | ".join(v.message for v in verdicts)
    if all(v.ok for v in verdicts):
        return _ok("C16", message)
    return _red("C16", message)


# C17 — ПОРОГ ВКЛЮЧЕНИЯ КЭША (спека 2026-08-19 §2.3, требование владельца).
#
# Отдельной строкой, а не внутри C16, и это не аккуратность ради аккуратности.
# У дрейфа и у порога РАЗНЫЕ причины, разные лекарства и разная тяжесть: дрейф
# это «выросло, посмотри», порог это «кэша нет, клиента подключать нельзя».
# Склеенные в одну строку, они дают отчёт, по которому нельзя понять, что
# именно чинить, — а чинят их в разных файлах.
#
# Мотив владельца именно за онбординг: это ЕДИНСТВЕННОЕ место, где дефект
# ловится ДО клиента. У `demo` префикс классификатора ниже порога СЕГОДНЯ, он
# уже на haiku, и кэш у него выключен молча с самого перевода — никто не
# заметил, потому что заметить было нечем.

def _c17(ctx: _Ctx) -> CheckResult:
    if ctx.config is None:
        return _blocked("C17", (
            f"клиент не поднимается ({ctx.config_error}) — префикс "
            f"классификатора собрать не из чего; сперва C1"), file="settings.yaml")
    if ctx.token_counter is None:
        # Тот же приём, что в C16: «ЗАМЕР НЕ ВЫПОЛНЕН» первым словом. Строка,
        # молчащая о непроверенном пороге, читалась бы как «порог пройден» —
        # то есть ровно как зелёная ширма на месте живого дефекта.
        return _red("C17", (
            "ЗАМЕР НЕ ВЫПОЛНЕН: счётчик токенов не задан (офлайн-прогон). "
            "Префикс классификатора НЕ сверялся с порогом включения кэша. "
            "Прогони с ANTHROPIC_API_KEY в окружении"))
    v = _verdict_of(ctx, CACHE_THRESHOLD)
    if v is None:
        return _blocked("C17", "сторож не вернул вердикта о пороге кэша")
    return _ok("C17", v.message) if v.ok else _red("C17", v.message)
...
def run_checks(client_dir, report_document: dict, *, slug: str,
               token_counter=None) -> list[CheckResult]:
    """Ровно 17 вердиктов по каталогу клиента, всегда и в порядке C1…C17.

    Каталог только ЧИТАЕТСЯ: `--check` ходит в том числе по боевому
    `chatter/clients/<slug>`, где живут деньги клиента.

    `token_counter` — единственная дверь наружу (C16 и C17, спека Хайку §9/§2.3):
    `count_tokens(model, system_text) -> int`. Не задан — C16 честно говорит,
    что замер не выполнен, и прогон остаётся полностью офлайновым. C17 при
    отсутствии счётчика говорит то же самое и тоже краснеет: непроверенный
    порог не имеет права выглядеть как пройденный.
    """
    try:
        ctx = _Ctx(client_dir, report_document, slug=slug, token_counter=token_counter)
    except Exception as exc:                              # noqa: BLE001
        return [_blocked(cid, f"контекст прогона не собрался: {type(exc).__name__}: {exc}")
                for cid in CHECK_IDS]

    if ctx.dir_error:
        # Каталога нет — прогон не состоялся ЦЕЛИКОМ. Это не «16 красных»:
...
def verdict(results, *, reviewed: bool) -> int:
    ...
    * **C12/C13/C16 на код не влияют** — они флаги (решение владельца): законная
