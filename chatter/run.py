...
def _escalation_pass(
    ...
    if deps.classify is not None:
        ...
        # P18 (дрил №3 2026-07-26): ответ бота ЭТОГО хода в историю ещё НЕ
        # записан — он уходит в транспорт позже. Без него классификатор судит
        # ход по одной реплике лида и собственных обещаний бота не видит:
        # «передам керівниці, вона підкаже вартість» превращалось в долг только
        # СЛЕДУЮЩИМ ходом, а оборвись диалог здесь — не превратилось бы вовсе.
        # Дописываем в ОКНО (не в store): история хранится один раз, дубля нет.
        if reply:
