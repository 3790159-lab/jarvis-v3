...
# 🔴 ЗАХВАТ НАСТОЯЩЕГО ЗАГРУЗЧИКА — не «текущего атрибута модуля».
#
# Значение по умолчанию `upload_file` в сигнатуре `run_client_backup`
# привязывается на этапе ОПРЕДЕЛЕНИЯ функции. Замер 04.09.2026: тест, который
# подменил `r2_storage.upload_file` и НЕ передал аргумент, продолжает
# пользоваться ОРИГИНАЛОМ — то есть привычный приём «подменить функцию в
# модуле» вокруг этой заливки НЕ РАБОТАЕТ. Застава, спрашивающая
# «а это `r2_storage.upload_file`?», такой случай пропустила бы и была бы
# декоративной. Поэтому здесь тот же объект, что стоит умолчанием.
_REAL_UPLOAD = r2_storage.upload_file

#: Признак тестового окружения. ЯВНЫЙ: переменную ставит сам pytest, кладя в
#: неё node id текущего теста. Замер обеих форм 04.09.2026 — в форме задачи
#: планировщика (`python -X utf8 scripts\state_backup.py`) её нет.
#: По `"pytest" in sys.modules` НЕ судим: это угадывание, он истинен всюду,
#: куда pytest попал импортом.
_PYTEST_MARKER = "PYTEST_CURRENT_TEST"


class LiveClientBackupBlocked(RuntimeError):
    """Суита попыталась залить клиентский набор в БОЕВОЙ бакет.

    🔴 НАМЕРЕННО ВНЕ ИЕРАРХИИ БЭКАПА и НЕ наследник `ClientBackupRefused`.
    `scripts/state_backup.py::main()` ловит `ClientBackupRefused` и трактует
    его как осознанное состояние НАСТРОЙКИ: rc остаётся 0, отличается лишь
    строка сводки. Будь застава наследником — её отказ стал бы тихой строчкой
    вместо аварии, то есть сторож утонул бы в широком `except`.

    Почему застава вообще нужна. Замер 04.09.2026:
    `tests/test_state_backup_script.py::test_main_success_...` зовёт настоящий
    `main()`, подменяя `run_backup`, `rotate_all_backups` и `send_telegram`,
    но НЕ подменяя `run_client_backup`. В worktree `.env` — ссылка на живой,
    ключи R2 боевые; `.secrets/*.db` там нет, а `requisites.yaml` копирует
    `new_worktree.py`. Итог: каждый полный прогон суиты заливал один объект и
    ПЕРЕЗАПИСЫВАЛ боевой `manifest.json` описанием из одной записи, оставляя
    клиентские базы в бакете НЕОПИСАННЫМИ. По датам: 01.09, 03.09 и 04.09
    испорчены, 02.09 уцелело — в тот день полного прогона после ночной задачи
    не было.

    Условие ДВОЙНОЕ, и это не осторожность: блокировать всё под pytest нельзя,
    вокруг заливки полно законных тестов, и они передают свой `upload_file`.
    Ошибка в обратную сторону — заблокировать ночную заливку — дороже:
    клиенты остались бы без бэкапа, и узнали бы мы об этом позже всех.
    Поэтому под заставой стоит отдельный встречный сторож
    (`test_without_the_pytest_marker_the_guard_stays_silent`).
    """
...
def run_client_backup(
    ...
    * **Ни одного «уже загружено» без сверки**: в конце — ``verify_uploaded``
    ...
    """
    # 🔴 ЗАСТАВА СТОИТ ПЕРВОЙ — РАНЬШЕ ФЕЙЛ-КЛОУЗА НА КЛЮЧЕ, и это не
    # придирка к порядку: на машине с ключом (а это наша машина) до неё
    # иначе просто не доходило бы, и суита продолжала бы портить бакет.
    #
    # Срабатывает только на ПЕРЕСЕЧЕНИИ двух условий:
    #   • явный признак тестового окружения (`_PYTEST_MARKER` — переменную
    #     ставит сам pytest, мы её не угадываем);
    #   • фактически используется НАСТОЯЩИЙ загрузчик — сравнение с
    #     `_REAL_UPLOAD`, объектом, а не с атрибутом модуля (см. его коммент).
    # Тест, передавший свой `upload_file`, проходит насквозь: законных тестов
    # вокруг заливки много, и ломать их запрещено.
    if os.environ.get(_PYTEST_MARKER) and upload_file is _REAL_UPLOAD:
        raise LiveClientBackupBlocked(
            "клиентская заливка запрещена из тестового окружения: признак "
            f"{_PYTEST_MARKER}={os.environ.get(_PYTEST_MARKER)!r} и настоящий "
            "загрузчик. Тест, которому нужна эта функция, обязан передать "
            "свой upload_file — иначе прогон перезаписывает БОЕВОЙ "
            "manifest.json и оставляет клиентские базы неописанными."
        )
