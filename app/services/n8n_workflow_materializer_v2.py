...
class N8NWorkflowMaterializerV2:
    def __init__(
        self,
    ...
    def build_known_good_payload(self, workflow_name: str, webhook_path: str) -> Dict[str, Any]:
        # known-good direct candidate: B1_v1_with_settings_empty
        return {
            "name": workflow_name,
            "nodes": [
                {
    ...
    def health(self) -> Dict[str, Any]:
        result: Dict[str, Any] = {
            "service": "n8n_workflow_materializer_v2",
    ...
    def debug_snapshot(self, workflow_name: Optional[str] = None, webhook_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        workflow_name = workflow_name or f"jarvis-v2-debug-{suffix}"
        webhook_path = (webhook_path or f"jarvis-v2-debug-{suffix}").strip("/")
    ...
    def create_workflow(self, *, name: Optional[str] = None, webhook_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        workflow_name = name or f"jarvis-v2-{suffix}"
        safe_webhook_path = (webhook_path or f"jarvis-v2-{suffix}").strip("/")
    ...
    def publish_workflow(self, *, name: Optional[str] = None, webhook_path: Optional[str] = None) -> Dict[str, Any]:
        created = self.create_workflow(name=name, webhook_path=webhook_path)
        activated = self.activate_workflow(created["workflow_id"])
        return {**created, "activate_response": activated}

    def probe_webhook(self, webhook_url: str, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        probe_payload = payload or {"source": "jarvis_probe_v2", "ts": int(time.time())}
...
_materializer_v2_singleton = None

def get_n8n_workflow_materializer_v2():
    global _materializer_v2_singleton
    if _materializer_v2_singleton is None:
        _materializer_v2_singleton = N8NWorkflowMaterializerV2()
    return _materializer_v2_singleton