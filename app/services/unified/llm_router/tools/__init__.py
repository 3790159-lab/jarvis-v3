...
# Явный список БЕСПЛАТНЫХ инструментов. Существует ровно затем, чтобы «не в
# карте» перестало означать «бесплатный по умолчанию»: молчаливый дефолт — это
# механизм, которым money-дыра 2026-07-08 вернётся (новый платный инструмент
# забыли вписать → роутер исполнит его без confirm и без guard_spend, а тесты
# останутся зелёными).
#
# Инвариант держит tests/test_router_paid_map_sync.py: каждый инструмент реестра
# обязан быть ЛИБО здесь, ЛИБО в _ROUTER_PAID_USD. Новый — красный тест, пока
# автор не примет решение осознанно.
_ROUTER_FREE = frozenset({
    "cancel_current_batch",      # отмена уже запущенного — ничего не запускает
    "get_user_stats",            # чтение леджера
    "swap_batch_set_quality",    # настройка параметра батча
    "swap_batch_start_source",   # приём исходника, генерации нет
    "swap_batch_start_targets",  # приём целей, генерации нет
})
