...
def build_cost_stats_tool(*, stats_fn: Optional[StatsFn] = None) -> Tool:
    ...
    return Tool(
        name="get_user_stats",
        description=(
            "Показать статистику расходов пользователя: сколько потрачено "
            "сегодня, за текущий месяц и за всё время. Данные берутся по самому "
            "пользователю, дополнительных параметров не нужно. Когда "
            "использовать: пользователь спрашивает про свои траты, расходы или "
            "статистику (например «сколько я потратил», «покажи статистику», "
            "«мои расходы за месяц»). Эквивалент /my_stats."
        ),
