...
CONFIG_PATH = Path(os.getenv("JARVIS_N8N_CONFIG_JSON") or (PROJECT_ROOT / "jarvis_stage3_artifacts" / "config" / "n8n_sidecar_runtime_config.json"))
APP_FILE = str(Path(__file__).resolve())

app = FastAPI(title="Jarvis N8N Stage B Final Sidecar", version="1.3.0")
...
class N8NStageBFinalSidecar:
    def __init__(self) -> None:
        cfg = load_runtime_config(CONFIG_PATH)

        env_api_key = os.getenv("N8N_API_KEY") or os.getenv("N8N_BEARER_TOKEN") or ""
        cfg_api_key = str(cfg.get("api_key") or "")

        # ENV FIRST, JSON SECOND
        self.base_url = (os.getenv("N8N_BASE_URL") or str(cfg.get("base_url") or "") or "https://daniliyc.app.n8n.cloud").rstrip("/")
        self.webhook_base_url = (os.getenv("N8N_WEBHOOK_BASE_URL") or str(cfg.get("webhook_base_url") or "") or self.base_url).rstrip("/")
        self.api_key = env_api_key or cfg_api_key
        self.timeout_seconds = int(os.getenv("N8N_TIMEOUT_SECONDS") or str(cfg.get("timeout_seconds") or "45"))

        self.config_source = "env" if env_api_key else ("json" if cfg_api_key else "none")
        self.env_has_api_key = bool(env_api_key)
        self.cfg_has_api_key = bool(cfg_api_key)
        self.api_key_length = len(self.api_key or "")
        self.api_key_preview = mask_secret(self.api_key or "")
    ...
    def config_snapshot(self) -> Dict[str, Any]:
        return {
            "service": "jarvis_n8n_stage_b_final_sidecar",
            ...
            "env_has_api_key": self.env_has_api_key,
            "cfg_has_api_key": self.cfg_has_api_key,
    ...
    def create_workflow(self, *, name: Optional[str] = None, webhook_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        workflow_name = name or f"jarvis-final-{suffix}"
        safe_webhook_path = (webhook_path or f"jarvis-final-{suffix}").strip("/")
