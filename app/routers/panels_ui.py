...
/* ПАЛИТРА «НІЧ» (вариант 02, выбран владельцем 13.08). Не новая тема, а
   затянутая нынешняя: фон уходит глубже, чтобы три уровня поверхности
   (фон / карточка / вложенный блок) наконец различались — раньше #0f1115 и
   #171a21 стояли слишком близко, и карточка не читалась как карточка. Синий
   стал холоднее и светлее: тёплый #4f8cff спорил с зелёным за внимание, а его
   роль — только «сюда можно нажать», не «важно». Правится ТОЛЬКО этот блок:
   разметка, классы и шкала кеглей палитре не подчиняются. */
:root{
  --bg:#0a0c10; --panel:#131720; --panel2:#1a1f2a; --line:#262c3a;
  --tx:#e7eaf0; --dim:#96a0b2; --acc:#6e9bff;
  /* Три смысла — три цвета, и ни один из них не занят ничем ещё: зелёный —
     ...
     означать «беда». */
  --ok:#34c97d; --warn:#efa92e; --bad:#ee5257;
  --calm:#7b8698; --off:#5a6273;
  --s1:#6e9bff; --s2:#efa92e; --s3:#34c97d;
}
...
/* Вторая строка — это ЧАСТЬ ответа («подниметься сам» / «поднимать некому»),
   а не служебная подпись. Кегль тот же (шкала не растягивается ради неё), но
   цвет обычного текста: рядом со служебной строкой одинакового тона обе
   читались как метаданные, и главное различение падения уезжало в шум. */
.sub.second{color:var(--tx)}
...
/* ДВЕ КОЛОНКИ ПО МЕСТУ, А НЕ ПО УСТРОЙСТВУ.
   Порог выведен из КОНТЕНТА, а не из модели телефона: 320 px — минимальная
   ширина, на которой карточка фермы ещё читается. Второй столбец появляется,
   когда для него хватает места: 320×2 + 14 gap + 2×18 padding = 690 px.
   Медиазапрос под конкретную ширину врал бы на любом другом устройстве, а Fold
   меняет ширину прямо во время просмотра — здесь раскладка меняется вместе с
   ним. Число 320 продублировано в _JS панели (порог свёртки состояния) — оно
   одно и то же по смыслу, поэтому и там записано как 690 = 320×2+14+36.

   Было 340 (порог 730). Замер владельца на РАЗВЁРНУТОМ Fold: 707 px, то есть
   контент 671 — второй колонке не хватало 23 px, и большой экран показывал ту
   же одну колонку, что и сложенный. 320×2+14 = 654 ≤ 671, запас 17 px.

   ⚠️ `min(320px,100%)`, а НЕ голые 320px: minmax(320px,·) не умеет сжиматься
   ниже своего минимума, и на внешнем экране Fold (344 px, контент 308) колонка
   осталась бы шире экрана, а страница уехала бы вбок — приёмка
   `panels_mobile_check --width 344` ловила это красным. */
...
/* Средний случай между `anywhere` и `nobreak`: переносить МОЖНО, но только по
   пробелам. Для колонок с человеческими фразами из нескольких слов: `nowrap`
   распёр бы их вбок, а `anywhere` рвёт посреди слова — найдено скриншотом
   приёмки журнала на 707 px, где четвёртая колонка ужала вторую до четырёх
   букв. Примеры фраз здесь НЕ цитируются намеренно: CSS уезжает в браузер
   инлайном, и цитата в комментарии стала бы вторым вхождением на странице —
   сторожа, считающие вхождения, покраснели бы на пустом месте. */
.wordsafe{overflow-wrap:normal}
...
# ─────────────────────────── ДВА ЯЗЫКА, НЕ i18n ─────────────────────────────
#
# Решение владельца 14.08: клиентская панель украинская (её читает клиент),
# панель Джарвиса — русская (она личная). Мультиязычности здесь НЕТ и не
# планируется: ни файлов переводов, ни ключей, ни выбора языка в рантайме.
# Различаются ровно три вещи — подпись состояния, суффикс возраста и атрибут
# `lang` у страницы, — и они записаны двумя словарями ниже. Всё остальное на
# каждой панели написано на своём языке прямо в её модуле.
#
# Украинский оставлен ДЕФОЛТОМ намеренно: клиентская панель не должна была
# измениться от этой правки ни на символ, и умолчание — самая дешёвая гарантия.
...
STATE_LABEL_RU = {"ok": "норма", "warn": "внимание", "bad": "сбой",
                  "off": "выключено"}
_LABELS = {"uk": STATE_LABEL, "ru": STATE_LABEL_RU}

# Суффиксы возраста: секунды, минуты, часы, сутки.
_AGO = {"uk": ("с тому", "хв тому", "год тому", "дн тому"),
        "ru": ("с назад", "мин назад", "ч назад", "дн назад")}
...
def dot_html(state: str, *, lang: str = "uk") -> str:
    """Цветная точка + слово. Оба, а не одно из двух."""
    labels = _LABELS.get(lang, STATE_LABEL)
    st = state if state in labels else "off"
    return (f"<span class='dot {STATE_TONE[st]}'></span>"
            f"<span class='slab'>{esc(labels[st])}</span>")
...
def ago(ts: float | None, now: float | None = None, *, lang: str = "uk") -> str:
    """Возраст факта. Панель без возраста врёт при первом зависании сборщика."""
    if not ts:
        return "—"
    sec, minute, hour, day = _AGO.get(lang, _AGO["uk"])
    d = (now or time.time()) - ts
    if d < 60:
        return f"{int(d)} {sec}"
    if d < 3600:
        return f"{int(d // 60)} {minute}"
    if d < 86400:
        return f"{int(d // 3600)} {hour}"
    return f"{int(d // 86400)} {day}"


def page(title: str, body: str, *, extra_js: str = "", lang: str = "uk") -> str:
    return (
