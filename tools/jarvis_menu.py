...
_DEVOPS = Category("devops", "🛠 Разработка", (
    ...
    _mi("/devtask_stats", "статистика конвейера за 30д", "exec", False),
    _mi("/ig_caption", "подпись для IG-поста (LLM)", "hint", False,
        hint="/ig_caption <тема> — сгенерирует подпись для Instagram-поста "
             "(2-4 предложения + эмодзи + хэштеги), платно (~$0.02, money-гейт)."),
    _mi("/ig_post", "опубликовать пост в Instagram (превью→тап)", "hint", False,
        hint="/ig_post <путь-или-last> <тема> — подготовит медиа, сгенерит "
             "подпись и покажет превью-карточку. Публикация в IG — только по "
             "тапу [📤 Опубликовать] (необратимо). Подпись платная (~$0.02)."),
    _mi("/ig_gen", "сгенерировать фото + пост в Instagram (тема→тап)", "hint", False,
        hint="/ig_gen [client=<клиент>] [nopersona] <тема> — сгенерирует фото "
             "по теме (через персону клиента с LoRA, если настроена в "
             "brand.md; nopersona — принудительно без персоны), подготовит "
             "медиа, сгенерит подпись и покажет превью-карточку — то же "
             "превью, что у /ig_post. Публикация в IG — только по тапу "
             "[📤 Опубликовать] (необратимо). Платно (~$0.08: фото + подпись)."),
    _mi("/ig_stats", "статистика IG-аккаунта и последних постов", "exec", False),
    _mi("/ig_schedule", "отложить публикацию поста (превью→очередь)", "hint", False,
        hint="/ig_schedule <аккаунт> <YYYY-MM-DD HH:MM> — берёт текущую "
             "подтверждённую превью-карточку (после /ig_gen или /ig_post) и "
             "ставит её в очередь на указанное время вместо немедленной "
             "публикации. Бесплатно — карточка уже оплачена, тут только "
             "постановка в очередь. Очередь проверяется каждые 5 минут."),
    _mi("/ig_queue", "очередь отложенных IG-постов ([Отмена])", "exec", False),
))
