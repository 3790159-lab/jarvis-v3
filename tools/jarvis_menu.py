...
_DEVOPS = Category("devops", "🛠 Разработка", (
    ...
    _mi("/ig_gen", "сгенерировать фото + пост в Instagram (тема→тап)", "hint", False,
        hint="/ig_gen <тема> — сгенерирует фото по теме, подготовит медиа, "
             "сгенерит подпись и покажет превью-карточку — то же превью, что у "
             "/ig_post. Публикация в IG — только по тапу [📤 Опубликовать] "
             "(необратимо). Платно (~$0.08: фото + подпись)."),
