...
BACKUP_DIR = PROJECT_ROOT / ("backup_stage3_conversational_brain_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
...
for path in [BOT_FILE, MEMORY_FILE, BRAIN_FILE, RESPONSES_FILE, MAIN_FILE]:
    if path.exists():
...
def local_fallback_reply(text: str) -> str:
    ...
    if "что ты умеешь" in t or "на что ты способен" in t:
        return (
            "Сейчас я умею работать как Jarvis Supervisor: создавать goal, "
            "вести mission lifecycle, запускать mission, показывать логи, "
            "помнить контекст диалога и отвечать в conversational режиме."
        )

    if "на каком мы этапе" in t or "какой следующий этап" in t:
        return (
            "Сейчас мы уже прошли базовый supervisor lifecycle и context memory. "
            "Текущий этап — conversational brain. Следом логично усиливать planner, "
            "LLM routing и auto-execution."
        )

    if "что это даст" in t:
        return (
            "Это даст более естественное общение с Jarvis: он сможет не только "
            "обрабатывать команды, но и свободно вести диалог, понимать вопросы, "
            "объяснять состояние системы и помогать планировать следующие шаги."
        )

    if "сможешь" in t or "можешь" in t:
        return (
            "Да, смогу. Базовый supervisor-контур уже собран, и теперь система "
            "готова к усилению conversational brain, routing и planner logic."
        )

    return (
        "Я понял твой вопрос. Сейчас у меня уже есть conversational brain в fallback-режиме. "
        "Если подключить Ollama или OpenAI, мои ответы станут заметно глубже и естественнее."
    )
...
def try_ollama_reply(text: str) -> str | None:
    try:
        prompt = (
            "Ты Jarvis V3 Supervisor Assistant. Отвечай по-русски, кратко, по делу, "
            "естественно и как инженерный AI-ассистент.\\n\\n"
            f"Контекст:\\n{build_context_block()}\\n\\n"
                ...
                "messages": [
                    {
                        ...
                        "content": (
                            ...
                            "естественно, полезно и инженерно-практично."
                        ),
                    },
                    {
                        "role": "user",
...
def generate_reply(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "mode": "conversation",
        "source": source,
        "confidence": 0.85 if source != "local_fallback" else 0.72,
    }
'''

responses_code = '''from __future__ import annotations
import app.core.bootstrap_utf8  # noqa: F401
...
from app.services.conversation_brain import generate_reply
...
def ask_responder(user_text: str):
    return post("/api/respond", {"text": user_text}, retries=2, timeout=45)
...
RESPONSES_FILE.write_text(responses_code, encoding="utf-8")
