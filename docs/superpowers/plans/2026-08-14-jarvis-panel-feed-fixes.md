...
> ## СТАТУС на 15.08: код готов, ЖДЁТ ОТМАШКИ НА МЕРЖ
>
> Ветка `arc/panel-feed-fixes`, в origin не пушена, **не смержена**.
> Задачи 1–7 выполнены, Task 8 выполнена в той части, что не требует деплоя.
>
> · **тесты** `tests/chatter/test_panel_feed.py` — 90 сторожей; вместе с
>   соседями панели **234 passed**;
> · **мутационный гейт DEV-26** — `Все 95 мутаций пойманы` (38 из них на ленту),
>   прогнан в основной сессии, дерево чистое;
> · **приёмка на ЖИВОМ логе** (не на стенде) — снимок «до» и «после» сняты на
>   живых источниках, лестница источников базы пройдена ступень за ступенью,
>   разметка отрисована на живой ленте, снимки приёмки пересняты.
>
> **Замер «до» → «после»** (живой лог, 421 строка / 31 677 байт):
>
> | | до | после |
> |---|---|---|
> | строк в ленте | 40 | 25 (окно = одно число) |
> | из них гардиан / клиент | 5 / 35 | 12 / 13 |
> | без времени | 5 | 0 |
> | дебаунс-строк | 2 | 0 |
> | кракозябр | строка «состав» нечитаема | 0 |
> | база | литерал `.secrets/demo.db` | у ЖИВОГО раннера, пояснение пустое |
> | прочитано лога | весь файл | окно 64 КБ |
>
> **Что ещё НЕ сделано и ждёт владельца:** сам мерж, явный рестарт бэкенда по
> точным PID и приёмка на поднятом процессе (шаги 2–4 Task 8). Мерж ≠ деплой.
>
> **Открытый вопрос владельцу:** квота окна между источниками (`_share_window`)
> — решение сверх спеки, принято по ходу; подробности в самой функции.
...
### Task 4: Путь к базе — у ЖИВОГО раннера, а не литералом

> ⚠️ **ЭТА ЗАДАЧА ПЕРЕПИСАНА ПО ХОДУ ИСПОЛНЕНИЯ.** Ниже сначала — почему первая
> редакция была неверна, потом утверждённая владельцем лестница. Код в шаге 3
> оставлен ИСТОРИЧЕСКИМ: он реализует первую редакцию и в дерево уже не годится.
> Фактическая реализация — коммиты `7c8ed85b` (первая редакция) и `9532dcab`
> (переделка) плюс правки по ревью.

**Файлы:**
- Изменить: `app/services/jarvis_farm.py`
- Тест: `tests/chatter/test_panel_feed.py`

**Почему первая редакция чинила не тот канал.** План велел выводить базу из
`chatter/clients/active.yaml` и уважать `CHATTER_DB` из окружения. На живой
машине это не работает:

- панель живёт в процессе `scripts/run_backend_detached.py`, раннер — в
  `python -m chatter.telethon_run`. Раннер — ребёнок `chatter_guardian_detached.ps1`,
  панель — ребёнок ДРУГОГО гардиана. Они **сиблинги**, окружение не общее.
  Замер 14.08: у раннера (PID 6864/9456) `CHATTER_PERSONAS='volska'`,
  `CHATTER_DB='.secrets\demo.db'`; у панели (PID 8908) обе `None`. В реестре
  USER/MACHINE их тоже нет;
- боевая подмена состава идёт **флаг-файлом**, а не правкой `active.yaml`:
  `git log -S yarina -- chatter/clients/active.yaml` пуст, а
  `chatter_guardian_detached.ps1:80-91` читает `state\chatter_demo_yarina.flag`.
  То есть единственный реальный случай смены первичного slug'а шёл по каналу,
  которого `active.yaml` не видит вовсе.

Сегодня базы совпадают **случайно**: и флаг Ольги, и `active.yaml` дают `demo`.

**Утверждённая владельцем лестница источников.** Возврат прежний,
`tuple[str | None, str]` = `(путь, пояснение)`; **пустое пояснение означает
«ответ достоверен»** и бывает только на ступенях 1–2:

```
1. TAMAPI_DB, затем CHATTER_DB своего окружения   -> (путь, "")   стенд / ручной запуск
2. ЖИВОЙ РАННЕР через psutil environ()            -> (путь, "")   ФАКТ
     CHATTER_DB, иначе первый непустой slug из CHATTER_PERSONAS
3. строка «состав: … db=…» из лога гардиана       -> (путь, пометка источника)
4. active.yaml -> .secrets/<первичный slug>.db    -> (путь, пометка источника)
     файла нет  -> путь + пометка про legacy-умолчание demo
     файл битый -> (None, пояснение)
```

Ограничения, которые нельзя нарушать:
- раннер ищется **существующим** механизмом файла — `_proc_table()` +
  `_launches(CHATTER_RUNNER, строка)` (грабля 1: запуск, а не упоминание).
  Второй способ искать процессы заводить запрещено;
- `psutil` импортируется **лениво, внутри функции**: панель не смеет умереть
  от его отсутствия;
- раннеров штатно **два PID**. Ответы совпали — берём; разошлись — это авария,
  и она обязана попасть в пояснение;
- `environ()` может бросить: `NoSuchProcess` — «раннер исчез, пока мы
  спрашивали», `AccessDenied` — «жив, спросить не удалось». Молча
  деградировать нельзя (DEV-18);
- относительные пути абсолютизируются от `ROOT` на **всех** ступенях, включая
  первую: cwd панели может быть любым.
...
> ⚠️ **Три поправки, вскрытые исполнением Task 3–4. Код ниже их уже НЕ отражает
> целиком — читать вместе с этой врезкой.**
>
> 1. **`if db_note`, а не `elif`.** По новой лестнице Task 4 непустое пояснение
>    приходит ВМЕСТЕ с рабочим путём (ступени 3–4 отвечают догадкой и честно
>    её называют). Старый `elif` в этом случае не читал бы базу вовсе: панель
>    показала бы одну строку-пояснение и пустую ленту — тишину там, где данные
>    есть. Пояснение и чтение базы независимы.
> 2. **`events(limit=40, table=None)` и таблица процессов насквозь.**
>    `client_db_path` умеет принять уже собранную таблицу (`_proc_table()` —
>    9.4 мс, весь `client_db_path` с таблицей — 0.9 мс против 10.7 мс без неё).
>    Таблицу строит `snapshot_fast()`, а `events()` живёт в `snapshot_slow()`,
>    у которой её нет. Либо протянуть таблицу из `snapshot_fast` в
>    `snapshot_slow`, либо признать параметр декоративным — но молча оставлять
>    его непроверяемым нельзя.
> 3. **Голод ленты: гардиан вытесняется клиентскими событиями.** Замер ДО правок
>    на живых источниках: в ленте 40 строк, из них 35 — `control_events` базы и
>    только 5 — гардиан, причём 2 из этих 5 дебаунсные. Причина не в фильтре: обе
>    пачки складываются в один список и режутся общим `limit`. Сортировка по
>    времени этого сама не чинит — если клиентских событий свежее, гардиан
>    снова уедет за срез. Нужен сторож на то, что ОБА источника доезжают до
>    ленты, и решение владельца, если квоты расходятся.
