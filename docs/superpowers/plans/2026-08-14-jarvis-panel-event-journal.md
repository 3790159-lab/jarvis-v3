...
- [x] **Шаг 4: прогнать оба гейта** — ПРОГНАНЫ 15.08: сторож 100/100, панели 126/126.
...
- [x] **Шаг 3: живое падение и подъём** — `down` 17:43:06, `recovered` 17:44:09, раннер вернулся за 80 с.
...
- [x] **Шаг 4: мёртвый писатель** — маркер состарен на месте вместо остановки таска (тот же сигнал, без слепой зоны).
...
- [x] **Шаг 5: ротация** — 5240 → 5001, маркер назвал и потолок, и схлопнутую прежнюю отметку.
...
- [x] **Шаг 6: бюджет первого экрана с полным журналом** — 51 мс из 130 на потолке; §4.5 не нужен.
...
- [x] **Шаг 7: скриншоты приёмки** — сняты; нашли дефект вёрстки, см. ниже. на 707 px и 344 px, как в заходе 1.
...
## Приёмка 15.08: что нашлось сверх плана

**Скриншот поймал то, что не поймали 32 сторожа.** На 707 px четвёртая колонка
забирала ширину у «Детали»: имена проб рисовались как «wor/ktre/e» и
«chat/ter_r/unn/er», подписи — как «нова/я при/чина». Первая правка (`<wbr>` +
`wordsafe`) вылечила имена и ужала «Деталь» до шести букв — строка в тридцать
этажей. Итог: колонок ТРИ, вид записи живёт под именем пробы (идиома
`_arc_table` из того же файла), деталь получила ширину. Четыре мутации в гейт.

**Гейт мутирует ЖИВОЙ файл — и это стоило записи в настоящем журнале.** Первый
прогон `mutate_ops_watchdog.py` оставил в `state/panel_events.jsonl` маркер
`rotated` с текстом «отброшено » — без причины, потому что его записал мутант.
Данные целы (ни одна настоящая запись не потеряна), но в журнале владельца
лежит ЛОЖНОЕ утверждение — ровно тот класс, ради которого журнал заведён.
Запись от 15.08 17:16:48 всё ещё там; вычищать её не стали, чтобы не гонять
read-modify-write с живым писателем ради одной косметической строки.

**Слепой сторож находится только прогоном.** Два из ста: (1) мутация на
`journal_append` проверялась тестом, который зовёт `note_trim_health` напрямую
и до писателя не доходит; (2) сторож порога читал ТУ ЖЕ константу, что и код, —
подъём порога до миллиарда он пережил, просто идя 14 минут 47 секунд. Второе
вылечено разделением: механизм гоняется на явном маленьком пороге, прод-число
пинует отдельный сторож.

**Ротация без перехода не случается.** Плановый шаг 5 («набить 5200 и запустить
сторожа») дал бы ложный красный: обрезка идёт только в момент дозаписи, а
дозапись — только при переходе. На чистом дереве переходов нет. Переход в дриле
создан явно и снят сразу.

## Находка на живых данных: `worktree` шумит алертами

За час в журнале оказалось 19 записей `changed` по одной пробе, и каждая ушла
владельцу как 🚨 «Новая причина». Ключ дедупа у `probe_worktree` включает СПИСОК
ГРЯЗНЫХ ФАЙЛОВ, поэтому причина меняется на каждом `git add`:

```
16:31:59  reason='branch:arc/panel-event-journal'
16:32:31  reason='...; dirty:app/services/jarvis_farm.py|tests/chatter/test_panel_journal_view.py'
16:33:34  reason='branch:arc/panel-event-journal'
```

Пути в ключе лежат НАМЕРЕННО и с обоснованием (счётчик не различил бы «один
файл сменился другим»), но цена этого решения до журнала не была видна никому.
Проба под мутационным гейтом; трогать её без отмашки владельца не стали.
