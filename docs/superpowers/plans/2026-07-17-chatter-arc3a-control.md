...
async def decide_outgoing(msg_id: int, *, registry, grace_seconds: float) -> str:
    ...
    ГОНКА: Telethon может разослать апдейт хендлерам РАНЬШЕ, чем send_message
    вернёт нам id — порядок его внутренняя деталь. Проигранная гонка = Аня
    опознаёт своё сообщение как чужое, глушит сама себя и молчит навсегда,
    тихо. Поэтому неопознанный id не решается мгновенно: ждём грейс и
    перепроверяем. 2 секунды невидимы на фоне её ритма в 30-50с."""
