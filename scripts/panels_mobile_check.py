...
PROBE = """() => {
  ...
  return {vw, scroll: document.documentElement.scrollWidth,
          ...
          // Считаем колонки, которые ЕСТЬ НА ЭКРАНЕ, а не объявленные треки:
          // `auto-fit` при 1024 px объявляет третий трек и тут же схлопывает его
          // в 0px (третьего ребёнка нет), а два живых забирают всю ширину
          // (замер: 487px 487px 0px). Прежний счёт по длине списка называл это
          // «три колонки» и краснел на раскладке, которой на экране нет.
          cols: two ? getComputedStyle(two).gridTemplateColumns.split(' ')
                        .filter(function(w){return parseFloat(w) > 0}).length : 0,
          grid: two ? getComputedStyle(two).display : ''};
}"""

# Ниже этой ширины второй колонке негде поместиться (320×2 + 14 gap + 36
# padding), выше — она обязана появиться. Числа выведены из ширины колонки,
# см. `.two` в panels_ui.CSS.
#
# 690, а не 730: замер владельца на развёрнутом Fold дал 707 px — прежний порог
# оставлял большой экран одноколоночным, и разворот телефона ничего не менял.
TWO_COLUMN_AT = 690
