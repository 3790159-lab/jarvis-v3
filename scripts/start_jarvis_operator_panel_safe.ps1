...
$ServerPath = Join-Path $ScriptsDir "jarvis_operator_panel_server.py"
$SmokePath  = Join-Path $ScriptsDir "jarvis_operator_panel_smoke.ps1"
...
$stdoutLog = Join-Path $LogsDir ("panel_stdout_" + $stamp + ".log")
$stderrLog = Join-Path $LogsDir ("panel_stderr_" + $stamp + ".log")
...
if ($LASTEXITCODE -ne 0) {
    throw "Python compile failed for panel server: $ServerPath"
}
...
if (-not $metaOk) {
    Write-Warning "Panel did not become ready."
    ...
    throw "Panel startup failed. See logs above."
}
...
if (Test-Path $SmokePath) {
    Write-Host ""
    Write-Host "=== PANEL SMOKE ===" -ForegroundColor Green
