...
#
# До вывода ПАНЕЛИ объявление не достаёт: панель поднимается через
# `Start-Process` с редиректами (Start-Panel ниже), а в той цепочке консоли
# нет вовсе (спека DEV-59 §7). Строка стоит ради вывода самой ЗАДАЧИ, и здесь
# это не формальность: журнал этого гардиана русскоязычный ЦЕЛИКОМ, а
# `Write-Host` из Write-G — ровно то, что читает человек, когда перезапускает
# задачу руками, разбирая аварию.
#
