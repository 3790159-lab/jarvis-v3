...
MUTATIONS = [
    ...
    # ── чистка состояния: ветка «семейство в цикле НЕ мерилось» ────────────
    # Мишень обещана в прошлом заходе и тогда НЕ заведена: ловца ещё не
    # существовало, а мутация без ловца — заведомо слепая мишень, то есть
    # ровно то, что гейт обязан находить, а не производить.
    #
    # Граница ломается ДВУМЯ разными способами, и чинятся они по-разному,
    # поэтому мишеней две. Одной хватило бы только на половину: сторожей на
    # эту ветку тоже два, и парный существует ровно потому, что «починка»
    # первой половины через край ломает вторую.

    # ПОЛОВИНА ПЕРВАЯ — чистка стала БЕЗУСЛОВНОЙ. Снимок эскалаций может
    # отсутствовать целиком при ЗЕЛЁНОМ ростере (watchdog не на деплой-хосте),
    # и тогда «ключа нет в probes» означает не «слаг ушёл из ростера», а
    # «спросить было нечем». Вымытый `alerted` снимается только
    # восстановлением, которого никто не измерит, — и клиент, о котором
    # владельцу УЖЕ сказали, замолчит навсегда.
    ("чистка: состояние НЕИЗМЕРЕННОГО семейства вымыто заодно", WATCHDOG,
     "        if (family == ATTENTION_PROBE_PREFIX and family in families_live\n"
     "                and key not in live):",
     "        if family == ATTENTION_PROBE_PREFIX and key not in live:",
     T_ATT + "::test_the_state_of_an_UNMEASURED_family_is_left_alone"),

    # ПОЛОВИНА ВТОРАЯ — та же ветка, «починенная» ЧЕРЕЗ КРАЙ. Соблазн очевиден
    # и потому опасен: «нет проб эскалаций — не чистим вообще ничего». Раннеры
    # в этом же цикле измерены прекрасно, и их мёртвые записи остаются жить, а
    # мёртвая запись однажды прочтётся как чья-то. Правило обязано быть
    # ПОСЕМЕЙНЫМ, и первой мишени этого НЕ ВИДНО: на такой правке она зелена.
    ("чистка: неизмеренное семейство заморозило чистку РАННЕРОВ", WATCHDOG,
     "    families_live = {client_family_of(k) for k in live}",
     "    families_live = {client_family_of(k) for k in live}\n"
     "    if ATTENTION_PROBE_PREFIX not in families_live:\n"
     "        return state",
     T_ATT + "::test_an_unmeasured_family_does_not_freeze_the_runner_cleanup"),
]
...
def write_mutant(path: Path, data) -> None:
    """Записать мутанта в ДВОИЧНОМ режиме и выдать ему СВОЙ mtime.

    Второй параметр — ТЕКСТ ИЛИ БАЙТЫ. Двух форм не потому, что лень выбрать:
    это ОБЩИЙ контракт писателя для всего парка гейтов, и его пинит
    `tests/test_mutation_gate_bytecode.py` — межгейтовый дисциплинарный файл,
    который зовёт `write_mutant(<путь>, <текст>)` у КАЖДОГО найденного гейта.

    🔴 ЧЕМ ЭТО СТОИЛО. Когда писатель тут принимал только байты, тот файл не
    смог позвать этот гейт вовсе — `TypeError: memoryview: a bytes-like object
    is required, not str`. Гейт проверку на чужой байткод НЕ ПРОВАЛИЛ, он из
    неё ВЫШЕЛ: защита осталась никем не измеренной, а гейт продолжал
    рапортовать «все мутации пойманы». Это тот же класс, что уже случился
    20.08 с `mutate_panel_client_supervisor` и ради которого мета-сторож
    «каждый гейт обязан быть ФАКТИЧЕСКИ измерен» и написан. Ужать общий
    контракт под удобство одного гейта — значит уменьшить то, что он
    охраняет, на этот гейт.
    ...
    if isinstance(data, str):
        # Кодируем САМИ и пишем двоично — не `write_text`. Текст, отданный
        # сюда строкой, попадает на диск ровно теми байтами, что в нём стоят:
        # переводы строк не трогаются НИ В ОДНОЙ из двух форм, иначе форма
        # вызова молча меняла бы содержимое файла.
