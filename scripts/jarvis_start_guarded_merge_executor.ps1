...
$StdOutLog = Join-Path $LogsDir "jarvis_guarded_merge_executor_stdout.log"
$StdErrLog = Join-Path $LogsDir "jarvis_guarded_merge_executor_stderr.log"
...
& '$PyExe' -X utf8 -m uvicorn app.jarvis_guarded_merge_executor:app --host $HostName --port $Port
"@
...
if (-not $Health) {
    throw "Guarded merge executor did not become healthy in time."
}

Write-Host ""
Write-Host "=== GUARDED MERGE EXECUTOR LIVE HEALTH ===" -ForegroundColor Green
