...
MUTATIONS = [
    ...
    ("отсутствие вердикта перестало быть проблемой", PS,
     # 🔴 ПЕРЕВОД СТРОКИ ЗДЕСЬ `\r\n`, А НЕ `\n`. В этой репе `.py` лежат в LF,
     # а `.ps1` — целиком в CRLF (замер: 191 CRLF, одиночных LF ноль), потому
     # что PowerShell 5.1 без BOM читает файл как cp1251. Многострочный фрагмент
     # с `\n` в `.ps1` НЕ НАЙДЁТСЯ никогда. Поймано это только потому, что гейт
     # считает «фрагмент не найден» СЛЕПЫМ, а не «ok»: мутация, не изменившая
     # файл, не проверила ничего, и молчаливое «прошло» было бы шестым способом
     # соврать.
