...
$StdOutLog = Join-Path $LogsDir "jarvis_supervisor_core_stdout.log"
$StdErrLog = Join-Path $LogsDir "jarvis_supervisor_core_stderr.log"
...
& '$PyExe' -X utf8 -m uvicorn app.main:app --host $HostName --port $Port
"@
...
if (-not $Health) {
    throw "Supervisor did not become healthy in time."
}

$global:LASTEXITCODE = 0
Write-Host ""
Write-Host "=== SUPERVISOR LIVE HEALTH ===" -ForegroundColor Green
