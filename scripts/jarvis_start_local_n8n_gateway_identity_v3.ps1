...
$StdOutLog = Join-Path $LogsDir "jarvis_local_n8n_gateway_identity_v3_stdout.log"
$StdErrLog = Join-Path $LogsDir "jarvis_local_n8n_gateway_identity_v3_stderr.log"
...
& '$PyExe' -X utf8 -m uvicorn app.jarvis_local_n8n_gateway_identity_v3:app --host $HostName --port $Port
"@
...
Write-Host "Waiting for identity gateway..." -ForegroundColor Yellow
...
if (-not $health) {
    Write-Host "Identity gateway did not become healthy in time." -ForegroundColor Red
...
Write-Host ""
Write-Host "=== IDENTITY GATEWAY HEALTH ===" -ForegroundColor Green
