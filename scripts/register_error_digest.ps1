# Registers (or re-registers) the JarvisErrorDigest scheduled task.
# Mirrors JarvisMorningDigest/JarvisIgTokenRefresh: S4U + RunLevel Highest so
# it runs whether the user is logged on or not (session-independent,
# reboot-resistant). StartWhenAvailable catches up a run skipped while the
# PC was off.
#
# Registration does NOT run it now -- run once to verify:
#   schtasks /Run /TN JarvisErrorDigest
# or wait for the daily 21:00 trigger.

$ErrorActionPreference = 'Stop'
$TaskName = 'JarvisErrorDigest'
$Python   = 'C:\jarvis\.venv\Scripts\python.exe'
$Script   = 'C:\jarvis\scripts\error_digest.py'
...
Write-Host "[OK] Registered scheduled task '$TaskName' (S4U / Highest, Daily 21:00)"
