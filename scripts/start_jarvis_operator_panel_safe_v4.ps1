...
$ServerPath = Join-Path $ScriptsDir "jarvis_operator_panel_server_v4.py"
$SmokePath  = Join-Path $ScriptsDir "jarvis_operator_panel_smoke_v4.ps1"
...
if (-not (Test-Path $ServerPath)) {
    throw "Panel server v4 not found: $ServerPath"
}

$stamp     = Get-Date -Format "yyyyMMdd_HHmmss"
$stdoutLog = Join-Path $LogsDir ("panel_v4_stdout_" + $stamp + ".log")
$stderrLog = Join-Path $LogsDir ("panel_v4_stderr_" + $stamp + ".log")
...
if ($LASTEXITCODE -ne 0) {
    throw "Python compile failed for panel server v4: $ServerPath"
}
...
if (-not $proc) {
    throw "Failed to start panel v4 process."
}
...
if (-not $metaOk) {
    Write-Warning "Panel v4 did not become ready."
    Write-Host ""
    ...
    throw "Panel v4 startup failed. See logs above."
}

Write-Host ""
Write-Host "Panel v4 is ready." -ForegroundColor Green
...
if ($RunSmoke -and (Test-Path $SmokePath)) {
    Write-Host ""
    Write-Host "=== PANEL V4 SMOKE ===" -ForegroundColor Green
