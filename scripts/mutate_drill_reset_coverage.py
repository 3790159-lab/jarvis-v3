...
MUTATIONS = [
    ...
    # ВЕРНУЛАСЬ после того, как автор сторожей закрыл дыру. На первом прогоне
    # гейта эта мутация была СЛЕПОЙ: детектор дублей можно было выкинуть целиком,
    # и суита не замечала. Сторож по ДАННЫМ ловит автора, положившего таблицу в
    # два списка сегодня; детектор нужен для другого случая — база и код
    # разъехались на живой машине, и сверка обязана сказать это словами ДО того,
    # как сброс что-то тронет.
    ("детектор дублей вырезан целиком (та самая дыра)", DR,
     [(b'        if len(cats) > 1:', b'        if False:')],
     G + "::test_table_named_in_two_categories_refuses_before_any_write"),
