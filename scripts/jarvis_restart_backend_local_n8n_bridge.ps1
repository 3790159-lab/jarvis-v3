...
$StdOutLog = Join-Path $LogsDir "backend_local_n8n_bridge_stdout.log"
$StdErrLog = Join-Path $LogsDir "backend_local_n8n_bridge_stderr.log"
...
& '$PyExe' -X utf8 -m uvicorn app.main_local_n8n_bridge:app --host $HostName --port $BackendPort
"@
...
try {
    Write-Host ""
    Write-Host "=== JARVIS LOCAL N8N HEALTH ===" -ForegroundColor Green
    $n8n = Invoke-RestMethod -Method GET -Uri "http://$HostName`:$BackendPort/api/jarvis/n8n/health" -TimeoutSec 10
    $n8n | ConvertTo-Json -Depth 20
} catch {
    Write-Host "Local n8n route check failed: $($_.Exception.Message)" -ForegroundColor Yellow
}
