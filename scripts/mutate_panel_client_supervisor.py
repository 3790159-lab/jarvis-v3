...
MUTATIONS = [
    ...
    # Найдено ЖИВОЙ ПРИЁМКОЙ 20.08 на стенде accept:8013, не сторожами и не
    # этим гейтом: причина доехала в журнал абракадаброй. Мутация — ровно та
    # правдоподобная «уборка дубля», которой дефект и был: `-Encoding` у
    # чтения выглядит лишним рядом с `-Encoding` у записи. Сторож на РАЗЛИЧИЕ
    # причин (две мутации выше) её не ловит по построению: две разные
    # абракадабры — тоже разные. Ловит только сторож на РАВЕНСТВО, и он живой,
    # процессом.
    ("чтение причины вернулось к системной кодовой странице: UTF-8 читается "
     "cp1251, в журнале абракадабра вместо причины", G,
     "        $lines = @(Get-Content -LiteralPath $pOut -Tail $MaxLines "
     "-Encoding UTF8 -ErrorAction Stop |",
     "        $lines = @(Get-Content -LiteralPath $pOut -Tail $MaxLines "
     "-ErrorAction Stop |",
     TL + "::test_the_reason_reaches_the_journal_readable"),
    ...
    # 🔴 ИСТОРИЯ ЭТОЙ СТРОКИ, и она стоит того, чтобы её не стирать.
    # Первый полный прогон дал ровно одну слепую мутацию — вот эту: утечка
    # пути НАСТОЯЩИМ (виндовым) видом, а адресный сторож
    # `test_health_does_not_leak_the_database_path` оставался ЗЕЛЁНЫМ. Причина
    # была замерена, а не выведена: тело ответа — JSON, а JSON УДВАИВАЕТ
    # каждую обратную косую, поэтому написание пути в теле и в переменной
    # РАЗНОЕ, и оба утверждения сторожа промахивались ПО ПОСТРОЕНИЮ.
    #
    #     db   = C:\Users\...\yarina.db
    #     body = {"ok": true, "db": "C:\\Users\\...\\yarina.db"}
    #
    # Сторож починен (d4e2d3a1): сверка идёт по РАЗОБРАННОМУ json, написания
    # перебираются явным списком. Замерено фактом — виндовую форму он теперь
    # ВИДИТ. Поэтому мутация нацелена обратно на него, а не на соседа
    # `test_health_says_exactly_one_thing`, который тоже краснеет: сосед,
    # названный вместо предмета, промолчал бы о том, что адресного сторожа
    # ослабили или удалили, — а его уже один раз приходилось чинить.
    ("/health отдаёт путь к базе клиента виндовым видом — тем самым, "
     "которого адресный сторож не видел до починки", A,
     '        return {"ok": True}',
     "        import os\n"
    ...
    ("ЖИВОЙ вердикт: no_bind_address схлопнут в no_response — гардиан "
     ...
     "        return 'no_response'\n"
     "    }",
     TG + "::test_a_tailnet_fallback_to_the_loopback_is_never_called_dead"),
