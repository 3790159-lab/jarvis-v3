...
$StdOutLog = Join-Path $LogsDir "backend_stage_b_v2_stdout.log"
$StdErrLog = Join-Path $LogsDir "backend_stage_b_v2_stderr.log"
...
& '$PyExe' -X utf8 -m uvicorn app.main_stage_b_v2:app --host $HostName --port $BackendPort
"@
...
Write-Host "Waiting for backend wrapper..." -ForegroundColor Yellow
...
if (-not $health) {
    Write-Host "Wrapper backend did not become healthy in time." -ForegroundColor Red
...
try {
    Write-Host ""
    Write-Host "=== V2 HEALTH ===" -ForegroundColor Green
    $v2 = Invoke-RestMethod -Method GET -Uri "http://$HostName`:$BackendPort/api/n8n/materializer-v2/health" -TimeoutSec 10
    $v2 | ConvertTo-Json -Depth 20
} catch {
    Write-Host "V2 health check failed: $($_.Exception.Message)" -ForegroundColor Yellow
}
