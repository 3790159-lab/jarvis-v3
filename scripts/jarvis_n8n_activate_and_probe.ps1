...
Write-Host ""
Write-Host "=== STEP 1: Check API ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "=== STEP 2: Activate workflow if needed ===" -ForegroundColor Cyan

if (-not $Match.active) {
    try {
        $ActivationResult = Invoke-RestMethod `
            -Method POST `
        ...
        $ActivationResult = [pscustomobject]@{
...
elseif ($null -ne $RefreshedWorkflow.active -and $RefreshedWorkflow.active -eq $true) {
    throw "Workflow is active, but production webhook still returns 404. Next step: open this workflow in n8n and click Publish."
}
else {
    throw "Workflow activation did not take effect. Inspect activation_result and refreshed_workflow."
}