...
MUTATIONS = [
    ...
    ("запуск снова садится на литеральное умолчание", RPC,
     ...
     GPORT + "::test_moving_the_number_in_the_registry_moves_the_launcher"),
    # ↑ МИШЕНЬ ПЕРЕЦЕЛЕНА после первого прогона. Была
    # `test_the_launcher_resolves_a_slug_port_from_the_registry`, и она
    # осталась ЗЕЛЁНОЙ: сторож зовёт резолвер `port_from_registry`
    # НАПРЯМУЮ, а мутация ломает место, где запуск этот резолвер
    # СПРАШИВАЕТ. Сторож честен, просто говорит о другом. Красное
    # обязан дать тот, кто проходит весь путь до `a.port`.
