...
$StdOutLog = Join-Path $LogsDir "jarvis_local_n8n_director_safe_v1_stdout.log"
$StdErrLog = Join-Path $LogsDir "jarvis_local_n8n_director_safe_v1_stderr.log"
...
& '$PyExe' -X utf8 -m uvicorn app.jarvis_local_n8n_director_safe_v1:app --host $HostName --port $Port
"@
...
Write-Host "Waiting for director..." -ForegroundColor Yellow
...
if (-not $health) {
    Write-Host "Director did not become healthy in time." -ForegroundColor Red
...
Write-Host ""
Write-Host "=== DIRECTOR SAFE V1 HEALTH ===" -ForegroundColor Green
