...
function Get-ExitCodeVerdict {
    ...
    # 🔴 ПРИВЕДЕНИЕ К 32 БИТАМ — ОБЯЗАТЕЛЬНО, И ВОТ ПОЧЕМУ.
    #
    # 18.08 00:58 пришёл первый настоящий код: -1. Строка назвала его «КРАХ
    # процесса (NTSTATUS)» — и соврала. В PowerShell 5.1 литерал `0xC0000000`
    # разбирается как Int32, то есть -1073741824, поэтому проверка
    # `($c -band 0xC0000000) -eq 0xC0000000` даёт True на ЛЮБОМ отрицательном
    # коде. Число в строке было правдиво, подпись — нет; это ровно тот класс,
    # который мы весь день ловим у других: догадка, выданная за наблюдение.
    #
    # Код выхода Windows — 32-битный БЕЗ знака. Приводим к нему сразу и
    # сравниваем беззнаково; `-1` становится 0xFFFFFFFF и попадает в свою
    # ветку, а не в чужую.
    ...
    switch ($u) {
        ...
        4294967295 {
            # 0xFFFFFFFF = TerminateProcess с кодом -1. Это НЕ крах и НЕ
            # taskkill: у первого код исключения, у второго 1. Сюда попадает
            # завершение чужой рукой с явным -1 — например, снос job object'а
            # вместе с упавшим приложением.
            return "код 0xFFFFFFFF (-1) - ВНЕШНЕЕ завершение с кодом -1: не taskkill (у него 1) и не крах"
        }
        default {
            # Диапазон ошибок NTSTATUS — 0xC0000000..0xC0FFFFFF (сюда попадают
            # 0xC0000005 access violation и 0xC0000409 fail-fast). Брать «всё,
            # что выше 0xC0000000» нельзя: тогда 0xFFFFFFFE снова читается как
            # крах — сторож поймал это на моей же правке.
            if ($u -ge 3221225472L -and $u -le 3238002687L) {
                return "код $hex - КРАХ процесса (NTSTATUS)"
            }
            return "код $u ($hex) - ни 0, ни 1: смотреть вручную"
        }
    }
}
