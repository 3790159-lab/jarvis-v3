...
# трубы относительно PYTHONUTF8 строкой выше (сторона ПИСАТЕЛЯ). Остаются ОБА:
# снять посредник, «раз объявление на месте», значит закрыть путь ничем.
#
# До вывода РАННЕРОВ объявление не достаёт: раннеры поднимаются через
# `Start-Process` с редиректами (Start-Runner ниже), а в той цепочке консоли
# нет вовсе (спека DEV-59 §7). Строка стоит ради вывода самой ЗАДАЧИ —
# `Write-Host` из Write-G, то есть ровно того, что читает человек, разбирая
# аварию руками. Журнал этого гардиана кодировку уже называет (`-Encoding
# utf8` ниже); консоль до сегодняшнего дня не называл никто.
#
