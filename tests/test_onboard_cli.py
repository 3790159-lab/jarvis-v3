...
def test_a_rebuild_over_the_reviewed_mark_is_refused(tmp_path, matching_brief):
    ...
    Проверяется ПОСЛЕДСТВИЕ, а не текст отказа: код «не состоялось» и файлы
    прошлой сборки на месте. Прогон, который переписал половину каталога и
    отказался на второй, оставил бы под подписью смесь двух сборок.
    """
    out = tmp_path / "build" / "onboard" / SLUG
