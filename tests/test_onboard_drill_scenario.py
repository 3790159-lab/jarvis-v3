...
def test_emoji_only_step_is_a_known_trap():
    """Ловит: сторожа DEV-32, который «прошёл бы» и на эмодзи-шаге.

    Шаг «🔥👍» существует в живом `yarina-onboarding-1-6.yaml` и в заготовку
    попасть НЕ ДОЛЖЕН (план T6). Здесь показано, что два теста ниже
    (`test_every_step_survives_normalization`,
    `test_every_step_is_matched_to_itself`) действительно кусаются: у такого шага
    нормализация пуста, а `match_step` возвращает `None` — то есть шаг обречён
    краснеть при идеально работающем боте.
    """
    trap = drill.Step(say="🔥👍", expect={"classifier_errors": 0})
    assert drill._normalize_say(trap.say) == ""
    assert drill.match_step(trap.say, (trap,)) is None
