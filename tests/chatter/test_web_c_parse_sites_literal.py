...
# ═══════════════════════════════════════════════════════════════════════════
# §9 сторож 13: ДВЕНАДЦАТЬ МЕСТ, КОТОРЫЕ ЛОМАЕТ СМЕНА ФОРМАТА — литерально
#
# Название точное, и точность тут не косметика (уточнено владельцем 28.08).
# Мест, которые РАЗБИРАЮТ структуру `contact_id`, в дереве больше двенадцати:
# `telethon_run.py::_persona_settings` берёт слуг хвостом и при удлинении
# головы работает как работал, а §4.2 вводит тринадцатое —
# `contact_id NOT LIKE '%:%:%'` в самой миграции. Оба лежат ВНЕ списка по
# ПРИЧИНЕ, а не по недосмотру, и причина записана у каждого в
# `COLON_PARSE_EXEMPT`. Список ниже — про то, что СЛОМАЕТСЯ, а не про то, что
# режет строку.
#
...
COLON_PARSE_EXEMPT: dict[str, str] = {
    ...
    # --- ТРИНАДЦАТОЕ место разбора, и оно названо вслух (владелец, 28.08).
    #     §4.2 требует `contact_id NOT LIKE '%:%:%'` — по определению самого
    #     §9 п.13 (`LIKE` в SQL перечислен формой) это разбор структуры. Оно
    #     вне списка двенадцати не по недосмотру: смена формата его не ломает,
    #     потому что это и есть код, СОВЕРШАЮЩИЙ смену формата.
    "chatter/storage/db.py::__init__ || SQL":
        "§4.2: место разбора, ВВЕДЁННОЕ миграцией и живущее ровно до её конца "
        "— идемпотентность по ФАКТУ, а не по файлу-маркеру",
    "chatter/storage/db.py::<module> || SQL":
        "§4.2: тот же разбор, если SQL миграции лежит константой модуля",
}
...
#
# ⚠️ СПЕКА НАЗЫВАЛА ВОСЕМЬ, А В ДЕРЕВЕ ИХ ДЕВЯТЬ. Девятое ПРИНЯТО владельцем
# 28.08, спека §3.2 правится отдельным коммитом; сторож держит девять. Девятое —
...
def test_guard14_perepis_sborki_derzhit_DEVYAT_mest_a_ne_vosem():
    ...
    Спека §3.2 называла ВОСЕМЬ (семь f-строк `telethon_run` + одна
    ...
    :126) в ценз не попала. Третье занижение переписи подряд по одному
    механизму (§8 п.3); девятое место ПРИНЯТО владельцем 28.08.

    Сторож считает по ДЕРЕВУ, а не по числу из спеки: иначе он зеленеет на
    восьми исправленных и девятой оставшейся — а девятая после каждого
    `--seed` возвращает стенду немигрированную форму.

    🔴 ЧИСЛО ПОЕХАЛО ВМЕСТЕ С АРКОЙ (28.08, тем же коммитом, что закрыл места).
    До кода сторож требовал ДЕВЯТЬ — это утверждение о дереве `9b5ff3f7`, и
    покраснеть ПОСЛЕ реализации оно было обязано, иначе арка не сделала ничего.
    Требование «девять» заменяется на «ноль» ровно так, как сторож сам и
    предписывал: «если их стало меньше — арка их закрыла и число обязано ехать
    вместе с ней».

    Но `== 0` в одиночку — пин, который оплачивается УДАЛЕНИЕМ: снести
    `_process_missed` целиком дешевле, чем провести её через владельца, а голый
    счётчик этих двух исходов не различает. Поэтому требований три, и второе с
    третьим — именно то, чего у пина «девять» не было:

      1. f-строчной сборки в переписанных местах не осталось (0 из 9);
      2. все 7 функций переписи ЖИВЫ — закрыть место, убрав его, нельзя;
      3. каждый файл переписи ДОТЯГИВАЕТСЯ до владельца сборки: «закрыто»
         означает «делегировано», а не «выброшено».
    """
    ...
    assert total == 0, (
        "в переписанных местах осталось %d сборок contact_id f-строкой:\n"
        "  %s\nКаждая знает форму наизусть и разъедется МОЛЧА в тот день, "
        "когда форма изменится: f-строка не бросает никогда."
        % (total, "\n  ".join("%s: %r" % (k, v)
                              for k, v in sorted(named.items()) if v)))

    gone = []
    for site in sorted(ASSEMBLY_SITES_9):
        rel, func = site.split("::")
        if not (REPO_ROOT / rel).is_file():
            gone.append("%s — файла нет" % site)
        elif func not in _defines(rel):
            gone.append("%s — функции нет" % site)
    assert not gone, (
        "%d мест переписи закрыты ИСЧЕЗНОВЕНИЕМ функции, а не переводом на "
        "владельца:\n  %s\nНоль f-строк, полученный сносом кода, который их "
        "содержал, — это ноль работы, а не закрытая арка."
        % (len(gone), "\n  ".join(gone)))

    mute = sorted({site.split("::")[0] for site in ASSEMBLY_SITES_9
                   if not _imports_owner(site.split("::")[0])})
    assert not mute, (
        "файлы переписи, которые f-строкой больше не собирают, но и владельца "
        "(%s) не импортируют: %s\nЗначит форма собирается там же наизусть, "
        "просто другой формой — а сторож 15 ловит только f-строку."
        % (", ".join(OWNER_MODULES), ", ".join(mute)))
