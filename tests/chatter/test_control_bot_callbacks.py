...
def _store(contact="42:demo", *, muted=False):
    s = Store(":memory:")
...
def test_resume_unmutes_and_reports():
    ...
    assert s.get_or_create_contact("42:demo")["paused"] == 0
...
def test_snooze_mutes_for_snooze_seconds():
    ...
    row = s.get_or_create_contact("42:demo")
    assert row["paused"] == 1
...
def test_snooze_creates_contact_if_absent():
    ...
    assert s.get_or_create_contact("99:demo")["paused"] == 1
...
def test_keep_records_event_without_muting():
    ...
    assert s.get_or_create_contact("42:demo")["paused"] == 0     # не трогаем паузу
...
def test_malformed_data_no_mutation():
    ...
    assert s.get_or_create_contact("42:demo")["paused"] == 1     # ничего не размутили
