...
def test_guard33_okno_zakryto_chestno_neprivodimye_stroki_eto_otkaz(tmp_path, bad, why):
    ...
    ⚠️ ЧТО ИМЕННО СЧИТАТЬ «НЕПРИВОДИМЫМ», СПЕКА НЕ ОПРЕДЕЛЯЕТ. Взято самое
    узкое честное чтение: значение, у которого текстовое представление НЕ
    РАВНО исходному номеру (REAL) либо его нет вовсе (BLOB). Оба хранимы в
    INTEGER-аффинной колонке сегодня, и оба меняют ключ идемпотентности молча
    — то есть ровно тот класс, ради которого §5 и написана. Если владелец
    арки понимает «неприводимое» иначе, поправить надо СПЕКУ, а не сторожа."""
    ...
    assert not isinstance(ei.value, AssertionError), (
        "перестройка проглотила неприводимое значение (%s) без отказа: ключ "
        "идемпотентности изменился молча." % why)
