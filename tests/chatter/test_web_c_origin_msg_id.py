...
@pytest.mark.parametrize("bad, why", [
    (7.5, "REAL: в колонке выживает как real, CAST даёт '7.5' — ключ, "
          "которого номером сообщения не бывает"),
...
def test_guard33_okno_zakryto_chestno_neprivodimye_stroki_eto_otkaz(tmp_path, bad, why):
    ...
    которого текстовое представление НЕ РАВНО исходному номеру (`REAL`) либо
    которого нет вовсе (`BLOB`). Оба меняют ключ идемпотентности МОЛЧА — то есть
    ровно тот класс, ради которого §5 и написана. `int` в этот список не входит:
    его приведение обратимо, и `Store` обязан его привести (сторож 30).

    🔴 ОБРАЗЕЦ ПОПРАВЛЕН 28.08 ПО ЗАМЕРУ. Первая редакция брала `7.0` и
    утверждала, что CAST даст `'7.0'`. Это неверно, и неверно НЕ У КОДА:
    INTEGER-аффинность сводит целочисленный REAL к INTEGER ПРИ ЗАПИСИ, и `7.0`
    ложится в колонку как `integer 7` — `typeof` даёт `'integer'`, CAST даёт `'7'`, то
    есть ТОТ ЖЕ ключ. Сторож требовал отказа в положении, которое САМ же и
    создать не мог, — проверка, отвечающая НЕ НА ТОТ вопрос. Образец заменён
    на `7.5`: дробный REAL в той же колонке выживает как `real` — замерено, а не
    выведено, и прибито соседним сторожем 33-б. Требование владельца
    «`REAL` — отказ» при этом НЕ ОСЛАБЛЕНО, а впервые исполнено живьём."""
...
# ═══ Сторож 33-б (встречная половина: пин на САМ ЗАМЕР) ══════════════════════

def test_guard33b_tselochislennyy_REAL_svoditsya_k_INTEGER_pri_zapisi(tmp_path):
    """Пин на допущение, из-за которого сторож 33 сначала требовал невозможного.

    Сторож 33 берёт `7.5`, а не `7.0`, и причина у выбора одна: аффинность
    колонки. Допущение живёт в SQLite, а не в нашем коде, — значит его обязан
    держать сторож, иначе первый же читатель «поправит» образец обратно на
    круглое число, тест снова станет требовать отказа там, где отказывать не
    от чего, и разбирать это будет уже кто-то третий.

    Утверждаем ровно два факта, и оба ЗАМЕРОМ:
      * `7.0` в INTEGER-аффинной колонке ложится как `integer` и CAST даёт
        `'7'` — ТОТ ЖЕ ключ, отказывать не в чем;
      * `7.5` в ней же выживает как `real` — есть чему быть неприводимым.
    """
    ...
    try:
        rows = []
        for i, v in enumerate((7.0, 7.5)):
            conn.execute(
                "INSERT INTO invoices (invoice_id, contact_id, currency, status,"
                " created_ts, created_by, origin_msg_id)"
                " VALUES (?, ?, 'USD', 'issued', 1000.0, 'bot', ?)",
                ("INV-volska-%06d" % i, CID, v))
        conn.commit()
        rows = conn.execute(
            "SELECT typeof(origin_msg_id) t, CAST(origin_msg_id AS TEXT) c"
            " FROM invoices ORDER BY invoice_id").fetchall()
    finally:
        conn.close()

    assert [r[0] for r in rows] == ["integer", "real"], (
        "аффинность колонки повела себя не так, как замерено 28.08: typeof дал "
        "%r, ждали ['integer', 'real']. Если целочисленный REAL перестал "
        "сводиться к INTEGER при записи — образец сторожа 33 обязан ехать "
        "обратно на 7.0, потому что тогда он ДЕЙСТВИТЕЛЬНО меняет ключ."
        % ([r[0] for r in rows],))
    assert rows[0][1] == "7", (
        "CAST от сохранённого 7.0 дал %r, а не '7': значит ключ всё-таки "
        "меняется, и 7.0 обязан быть в списке неприводимых." % (rows[0][1],))
