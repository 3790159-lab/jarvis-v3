...
SLUG = "demo"
# Телеграмный контакт сегодняшней формы — нужен только как ПРЕДПОСЫЛКА
# («регресса нет»), ни один сторож на нём не строится.
TG = "111:demo"
...
def _register(channel: str, deliverer):
    ...
    канала обязано стоить ОДНУ регистрацию."""
    mod = _core()
...
@pytest.mark.parametrize("alien, why", [
    ("111:foreign", "чужой слаг: диалог другого клиента"),
    ("999:demo", "контакта нет в базе этого клиента"),
...
def test_stend_deistvitelno_gonyaet_NEtelegramnyi_kanal(store):
    """Предпосылка всего файла: контакт `%s` не разбирается сегодняшним
    двухсегментным `contact_ref`, то есть это ДЕЙСТВИТЕЛЬНО другой канал.
