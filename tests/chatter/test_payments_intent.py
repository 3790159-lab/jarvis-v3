...
def test_a_number_far_from_the_alias_is_not_a_quantity(pricing):
    """«Нас троє співзасновників, потрібен логотип» — тройка про людей, а не про
    логотипы. Расстояние здесь единственный рубеж: число правдоподобное, и
    граница количества его пропускает. Ошибка стоит эскалации на пустом месте —
    бот увёл бы к владелице простой запрос как «три логотипа»."""
    intent = read_intent("Нас троє, три співзасновники, потрібен логотип", pricing)
...
@pytest.mark.parametrize("text", [
    # Прогон Т1 11.08: «Готовий замовити зараз?» дал котировку вместо счёта —
    # пары ("готов","замов") в списке не было. Заказать — это купить: лид,
    # сказавший «готовий замовити», уже принял решение, и ответ «орієнтовно
    # до $400» вместо счёта роняет продажу в самой тёплой точке.
    "Готовий замовити зараз?",
    "Готов заказать сейчас",
    "Хочу замовити логотип",
    "Хочу заказать логотип",
    # Готовность НАЧАТЬ работу — то же решение, другими словами. Пара с
    # «давайте» уже была, пара с «готов» — нет.
    "Готовий почати",
    "Готов начать",
    # «Оформляем» в паре: одиночная основа «оформ» тут запрещена намеренно,
    # см. тест про оформление соцсетей.
    "Давайте оформлювати",
    "Давайте оформлять",
    "Давайте замовимо",
    "Давайте закажем",
])
def test_readiness_to_buy_is_recognised_not_only_as_readiness_to_pay(pricing, text):
    """«Замовити» — это операция Б, а не вопрос цены.

    Спека говорит «явная готовность платить», и буквальное чтение оставило в
    списке только платёжные глаголы. Но лид не говорит «готовий платити» —
    он говорит «беру», «оформляємо», «готовий замовити»."""
    assert read_intent(text, pricing).wants_invoice
...
def test_one_word_purchase_signal_is_recognised(pricing, text):
    """Самые короткие ходы воронки — согласие одним глаголом. Пару им составить
    не из чего: второго слова в сообщении просто нет."""
    assert read_intent(text, pricing).wants_invoice


@pytest.mark.parametrize("text", [
    # «Беру» матчится ТОЧНОЙ словоформой, а не основой: основа поймала бы
    # «беручи до уваги». Но и точная словоформа живёт в оборотах, где решения
    # о покупке нет — их вычитаем явно.
    "Беру паузу, подумаю",
    "Беру до уваги, дякую",
    "Беру тайм-аут на подумать",
    # Ровно на этой фразе видно разницу между СЛОВОФОРМОЙ и основой: «беручи»
    # начинается с «беру», и матч основой выставил бы счёт деепричастию.
    "Беручи ваші роботи за приклад, зробіть щось схоже",
    # А это ловушка основы «оформ»: оформление соцсетей — обычная услуга
    # дизайнера, самое частое слово в этом прайсе. Основой ловить нельзя.
    "Оформлення соцмереж робите?",
    "Оформление страницы сколько стоит?",
])
def test_one_word_signal_does_not_fire_on_look_alike_phrases(pricing, text):
    """Ложный счёт здесь дороже пропущенного: карточка владельцу на пустом
    месте и «борг на клієнті», которого клиент на себя не брал."""
    assert not read_intent(text, pricing).wants_invoice
