...
def test_ULIKA_CELA_posle_snyatiya(store):
    ...
    # ДВЕ попытки, а не одна: отказ — это тоже состоявшаяся попытка доставки,
    # и она считается наравне с «сеть моргнула». Контракт об этом молчал, и
    # ожидание «терминальная не считается» было ДОПУЩЕНИЕМ сторожа, а не
    # договором; решено в пользу счёта (интегратор, 25.08). Утверждение теста
    # от этого не зависит вовсе — оно ниже, про то, что число ПЕРЕЖИВАЕТ
    # снятие; здесь число только фиксируется, чтобы «пережило» было проверяемо.
    assert before[0] == "refused" and before[3] == 2, before
...
# ── УСТУПЛЕНИЕ ЧЕЛОВЕКУ: ветка, которую не видно по молчанию ────────────────
#
# 🔴 НАЙДЕНО МУТАЦИОННЫМ ГЕЙТОМ, и находка важнее самой мутации.
#
# Гейт снял условие «уступаю человеку» ЦЕЛИКОМ — и сторож остался ЗЕЛЁНЫМ.
# Причина: открытие эпизода перехвата ставит `paused=1`, а сразу за этой
# веткой стоит общий гейт паузы, дающий ТУ ЖЕ ТИШИНУ. По молчанию две ветки
# неразличимы, значит ветку можно снести, и не покраснеет ничего
# ([[jarvis-guard-caught-dead-branch]]).
#
# Отсюда правило, по которому написаны сторожа ниже: ПИНИТЬ РАЗНИЦУ, А НЕ
# СОВПАДЕНИЕ. У ветки есть две вещи, которых нет у соседней:
#
# 1. СОБЫТИЕ `bot_yielded_to_human`. Без него разбор «почему бот замолчал на
#    этом ходу» не отличает «человек перехватил» от «пауза по любой другой
#    причине» — а лечатся они по-разному: первое проходит само, второе ждёт
#    действия владельца.
# 2. СНЯТЫЙ ИНДИКАТОР «печатает». Иначе бот висит «печатає…» ПОВЕРХ человека,
#    который уже ответил: лид видит, что собеседник как будто продолжает
#    набирать, и ждёт вторую реплику, которой не будет.

YIELD_EVENT = "bot_yielded_to_human"


def _bot_turn(store: Store, contact_id: str, *, reply: str):
    """Один ход бота в диалоге. Возвращает транспорт-двойник."""
    deps = _persona("demo", store, scripted=[reply]).deps
    transport = FakeConsoleTransport(preload=[], echo=False)
    process_batch(contact_id, ["а сколько стоит?"], transport, deps)
    return transport


class _InterruptingTransport(FakeConsoleTransport):
    """Транспорт, который дёргает крючок ПОСРЕДИ хода.

    Крючка два, и оба принадлежат humanizer'у: `read_acknowledge` («бот
    прочитал») и `send_typing` («бот печатает»). Оба вызываются ПОСЛЕ входа в
    обработку хода и ДО первого баббла — то есть ровно в том промежутке, в
    котором и живёт гонка.
    """

    def __init__(self, on_midturn):
        super().__init__(preload=[], echo=False)
        self._on_midturn = on_midturn
        self.midturn_hits = 0
...
def _bot_turn_interrupted_midway(store: Store, contact_id: str, *, reply: str):
    """Ход бота, ПОСРЕДИ которого человек нажимает «отправить».

    🔴 ПОЧЕМУ «ПОСРЕДИ» — ЕДИНСТВЕННЫЙ ГОДНЫЙ СЦЕНАРИЙ, и это знание куплено
    мутационным гейтом. На входе в ход стоит ОБЩИЙ гейт паузы: перехват,
    поставленный ДО вызова, отсекает ход там же, до генерации, и до ветки
    «уступаю человеку» дело не доходит вовсе. Тишина при этом есть, события
    нет — и сторож, ставящий перехват заранее, ЗЕЛЁН НА ЛЮБОЙ РЕАЛИЗАЦИИ,
    ВКЛЮЧАЯ ТУ, ГДЕ ВЕТКИ НЕТ СОВСЕМ. Ровно это гейт и предсказал.

    Ветка ловит ГОНКУ: бот начал сочинять, когда диалог был свободен, а
    человек нажал «отправить», пока шла пауза чтения-печати. Значит состояние
    обязано меняться МЕЖДУ входом в ход и отправкой первого баббла.

    Крючков поставлено три, и срабатывает первый попавшийся: пауза
    (`deps.sleep` — тот же приём, которым в дереве уже ловят «мут пришёл в
    середине ответа»), «прочитал» и «печатает». Три, а не один, потому что
    ПОРЯДОК этих вызовов внутри хода — деталь humanizer'а, а не договор: я не
    вправе требовать, чтобы ход начинался именно с паузы. Открытие эпизода
    идемпотентно, поэтому лишние срабатывания безвредны.
    """
    deps = _persona("demo", store, scripted=[reply]).deps
    opened: list[bool] = []

    def owner_jumps_in() -> None:
        if opened:
            return
        opened.append(
            _need("open_human_takeover")(store, contact_id, msg_id=4242,
                                         detail="я отвечу сам", now=1000.0))
    ...
    transport = _InterruptingTransport(owner_jumps_in)

    assert store.get_or_create_contact(contact_id)["paused"] == 0, (
        "предпосылка стенда: на ВХОДЕ в ход диалог свободен. Заглушённый "
        "заранее контакт отсекается общим гейтом паузы, и ветка «уступаю "
        "человеку» не исполняется вовсе")

    process_batch(contact_id, ["а сколько стоит?"], transport, deps)

    assert transport.midturn_hits or opened, (
        "ни один крючок середины хода не сработал: ход не дошёл ни до паузы, "
        "ни до «прочитал», ни до «печатает», и гонку воспроизвести нечем")
    assert opened and opened[0] is True, (
        "эпизод перехвата ПОСРЕДИ хода не открылся (%r): сравнивать нечего — "
        "сторож ниже был бы зелен по построению" % (opened,))
    assert store.get_or_create_contact(contact_id)["paused"] == 1, (
        "после хода контакт не заглушён: перехват до раннера не доехал")
    return transport


def _yields(store: Store) -> list[str]:
    """Кому бот уступил, по журналу."""
    rows = store._conn.execute(
        "SELECT contact_id FROM control_events WHERE kind=? ORDER BY id",
        (YIELD_EVENT,)).fetchall()
    return [r["contact_id"] for r in rows]


def test_bot_USTUPAET_cheloveku_vmeshavshemusya_POSREDI_hoda(store):
    ...
    🔴 ВЕТКА ДОСТИЖИМА ТОЛЬКО В СЕРЕДИНЕ ХОДА, и это надо было узнать
    мутационным гейтом. Перехват, поставленный ДО вызова, отсекается общим
    гейтом паузы на входе — до генерации, до ветки, до события. Тишина есть,
    ветки нет. Сторож, ставящий перехват заранее, зелен на любой реализации,
    включая ту, где ветки нет совсем; первая редакция этого файла была именно
    такой. Стенд поэтому двигает состояние ПОСРЕДИ хода — см.
    `_bot_turn_interrupted_midway`.

    🔴 ТРИ УТВЕРЖДЕНИЯ, И ПЕРВОЕ САМО ПО СЕБЕ НИЧЕГО НЕ СТЕРЕЖЁТ. Молчание
    даёт и соседняя ветка (общий гейт паузы), поэтому сторож на одно только
    «бот не сказал» пропускает УДАЛЕНИЕ ВСЕЙ ВЕТКИ. Уникальны у неё событие в
    журнале и снятый индикатор; они и стерегут.

    Второй контакт обязателен отдельно: реализация, заглушившая бота ВЕЗДЕ,
    прошла бы половину сторожа и выглядела бы как `kill_switch`, которого
    никто не нажимал.
    """
    held = _bot_turn_interrupted_midway(store, A, reply="НЕ ДОЛЖНО УЙТИ ЛИДУ")

    assert held.sent == [], (
        "бот договорил поверх человека, вмешавшегося посреди хода: клиент "
        "получит два разных ответа на один вопрос; %r" % (held.sent,))

    assert _yields(store) == [A], (
        "хода «уступаю человеку» нет в журнале (%r): по журналу этот ход "
        "неотличим от паузы по любой другой причине, а лечатся они "
        "по-разному — и, что хуже, саму ветку теперь можно снести, и не "
        "покраснеет ничего" % (_yields(store),))

    assert held.typing_events, (
        "индикатор «печатает» не тронут вовсе: бот остаётся «печатає…» поверх "
        "человека, который уже ответил, и лид ждёт вторую реплику")
    assert held.typing_events[-1] is False, (
        "последнее, что бот сделал с индикатором, — %r вместо снятия: лид "
        "видит собеседника, который как будто продолжает набирать"
        % (held.typing_events[-1],))

    free = _bot_turn(store, B, reply="Добрый день. Чем помочь?")
    assert free.sent, (
        "бот замолчал и во ВТОРОМ диалоге, где человека нет: перехват одного "
        "контакта выключил всю воронку")
    assert _yields(store) == [A], (
        "уступление записано и на диалог, где человека нет: %r" % (_yields(store),))


@pytest.mark.parametrize("setup, why", [
    (lambda s, cid: s.mute(cid, source="command", now=900.0),
     "пауза командой владельца"),
    (lambda s, cid: s.mute(cid, source="command", until=4000.0, now=900.0),
     "снуз «ещё 1 час» из карточки"),
    (lambda s, cid: s.set_runtime_flag("kill_switch", "1", ts=900.0),
     "глобальный рубильник"),
])
def test_ustuplenie_cheloveku_OTLICHIMO_ot_lyuboi_drugoi_tishiny(store, setup, why):
    """🔴 ПАРНАЯ ПОЛОВИНА, и без неё первый сторож зелен на чём угодно.

    Все эти состояния дают ТУ ЖЕ тишину, и реализация, пишущая
    `bot_yielded_to_human` на каждое молчание, прошла бы сторож выше целиком —
    оставаясь при этом враньём: журнал сообщал бы, что человек ведёт диалог,
    в котором его нет, и разбор «почему клиент остался без ответа» пошёл бы не
    туда.

    Разбито параметрами: одно спутанное состояние обязано называть СЕБЯ, а не
    тонуть в общем «событие пишется не тогда».
    """
    setup(store, A)
    turn = _bot_turn(store, A, reply="НЕ ДОЛЖНО УЙТИ ЛИДУ")

    assert turn.sent == [], (
        "предпосылка сторожа: %s тоже даёт тишину, иначе разницу сравнивать "
        "не с чем; %r" % (why, turn.sent))
    assert _yields(store) == [], (
        "%s записана как «бот уступил человеку» (%r): журнал утверждает, что "
        "диалог ведёт человек, которого там нет" % (why, _yields(store)))
