...
def test_unreadable_config_refuses_loudly_and_does_not_kill_the_screen(panel):
    ...
    # Проверяем ИМЕННО слот имени в шапке, а не наличие фразы где-нибудь на
    # странице: «конфіг клієнта не прочитано» уже стоит ниже, в подписи к
    # дневному лимиту (`cap_txt`), и голое `"не прочитано" in html` проходило
    # даже тогда, когда в шапку подставлен тихий дефолт. Поймано мутацией
    # «нечитаемый конфиг подставляет тихий дефолт вместо отказа».
    assert re.search(r"конфіг клієнта не прочитано\s*·\s*TAMAPI\s*·\s*клієнт\s*zeta",
                     html), (
        "конфиг не прочитан, а в шапке стоит не отказ, а что-то другое: "
        "пустое место или дефолт читаются клиентом как «так и задумано»")
