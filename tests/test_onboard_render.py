...
def test_the_address_stub_does_not_reuse_the_word_orientir(f):
    """Ловит: одно слово в двух смыслах внутри одного файла.

    «Орієнтир» жил у нас и как ориентир проезда (заглушка адреса), и как
    ориентир ПО ВАРТОСТІ в разделе про цены. C13 честно поднимала флаг
    «одна сущность или разные», и каждый новый клиент получал бы этот флаг на
    ровном месте. Решение владельца 17.08: это ОМОНИМ, сущности разные, и
    разводить их надо словами, а не проверкой.

    Сторож стоит на СЛОВЕ в заглушке, потому что именно оно уезжает лиду.
    """
    knowledge = f["knowledge.md"]
    stub_lines = [ln for ln in knowledge.splitlines() if "називає" in ln and "Адрес" in ln]
    assert stub_lines, "заглушки адреса нет вовсе — проверять нечего"
    for line in stub_lines:
        assert "орієнтир" not in line.casefold(), (
            f"«{line.strip()}» — то же слово, что «орієнтир по вартості»; "
            "C13 будет флагом на каждом клиенте")
        assert "як нас знайти" in line.casefold(), line.strip()
