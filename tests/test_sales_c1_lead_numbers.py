...
@pytest.mark.parametrize("reply", [
    "Ви називали бюджет $1000 — це і є фінальна сума за проєкт.",
    "Ви називали бюджет $1000 — загальна сума саме така.",
    "Вы называли бюджет $1000 — итоговая сумма именно такая.",
])
def test_lead_number_named_as_our_sum_stays_flagged(reply):
    """Дыра, найденная замером 05.09: вето знало «ціна/вартість/коштує», но НЕ
    знало слова «сума». Атрибуция при этом ЖИВА, поэтому все три формы
    проходили как пересказ — а это не пересказ, это назначение НАШЕЙ суммы
    числом лида. Бюджет лида становился нашим прайсом.

    Цена пропуска — не «некрасиво», а обещание: лид получает подтверждение,
    что работа стоит ровно столько, сколько он готов был отдать."""
    assert "price" in _rules(reply, lead=lead_numbers(LEAD_SAID_1000))
