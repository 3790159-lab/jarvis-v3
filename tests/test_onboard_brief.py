...
def test_working_hours_written_as_a_range_survive():
    """Класс ошибки: правило числовой заглушки съело часы работы «9-18».

    Ровно этот случай назван КОНТРОЛЕМ в спеке §1.2: ячейка из цифр и дефиса
    короче порога, но ответ настоящий, и из него берётся факт «выходные дни»
    (R7). Мутация «сдвинуть долю на 0.05» проходит зелёной без этого сторожа —
    то есть граница правила держалась комментарием автора.

    Цена ошибки односторонняя и высокая: срезанные часы уходят в раздел 3 как
    «в брифе нет», клиент получает заглушку «Графік — називає власник», и лид
    спрашивает время работы у человека, который заполнял форму именно затем,
    чтобы этого не было.
    """
    ok("9-18", "Графік роботи студії")


def test_a_long_digits_only_answer_is_not_a_stub():
    """Класс ошибки: правило числовой заглушки перестало смотреть на ДЛИНУ.

    Заглушка — это «1» или «1.0» в текстовом поле: человек не писал ответа.
    Телефон и ценовая вилка состоят из тех же цифр и пробелов, но короткими
    не бывают. Без ограничения длины доля цифр в «+380 67 123 45 67» даёт
    единицу, и контакт клиента уезжает в мусор вместе с причиной
    «числовая заглушка».
    """
    ok("+380 67 123 45 67", "Телефон для звʼязку")
