...
EXPECTED_ROUTES = {
    ...
    ("/ops/outgoing", ("GET",)),
    # Пятая ручка, арка «отправка из панели перестаёт быть телеграмной»
    # (спека 2026-08-28-web-d-panel-send.md §2.7). Страница ОДНОГО диалога:
    # лента разговора и поле ввода. Без неё смерженной 25.08 ручке
    # `/api/outgoing` негде было держать кнопку. ЗА `require_owner` (она на
    # роутере дашборда) и fail-closed по адресу: чужой слуг и контакт, которого
    # в базе нет, получают отказ, а не пустую ленту. Список правится РУКАМИ
    # намеренно — он затем и стоит, чтобы новая поверхность не приезжала молча.
    ("/panel/tamapi/d/{contact_id}", ("GET",)),
    ("/panel", ("GET",)),
    ("/panel/login", ("GET",)),
    ("/panel/login", ("POST",)),
    ("/panel/tamapi", ("GET",)),
