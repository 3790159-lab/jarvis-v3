...
@pytest.mark.parametrize("wires, case", [
    (TG_ONLY_RED, "три провода, один мёртв"),
    ({"tg_api": False, "llm_api": True}, "R2 не настроен — терма нет вовсе"),
])
def test_the_snapshot_yields_one_key_per_term_and_nothing_besides(wires, case):
    """Агрегат ловится ПО ФАКТУ ПОЯВЛЕНИЯ, а не по угаданному имени.

    🔴 ПЕРВАЯ РЕДАКЦИЯ ЭТОГО СТОРОЖА БЫЛА СЛЕПОЙ, и слепоту нашёл мутационный
    гейт: она перечисляла имена (`reachability`, `reach`), а мутация добавила
    рядом с тремя термами четвёртый ключ `network` — и сторож остался зелёным.
    Имён у агрегата бесконечно много: `network`, `online`, `connectivity`,
    `link_ok`. Перечисление их не покрывает НИКОГДА, а выглядит как покрытие —
    то есть само является зелёной лампой ([[jarvis-literal-lists-not-introspection]]).

    Считается ЧИСЛО: ключей, добавленных снимком, обязано быть ровно столько,
    сколько термов в самом снимке. Тогда ломается любое имя, а завтрашний
    четвёртый провод не проскочит незамеченным.

    Число берётся ИЗ СНИМКА, а не из `len(WIRES)`: состав проводов пинит
    сторож выше, а этот отвечает на другой вопрос — «один терм, один вердикт».
    Второй расклад (`R2` не настроен, терма нет вовсе) держит это честно:
    сторож, зашивший тройку, зеленел бы на любом числе проводов.

    Обе стороны неравенства — разные аварии, и названы они порознь: лишний ключ
    гасит красное соседней зелёной лампой, недостающий выдаёт «вердикта нет» за
    «всё хорошо».
    """
    expected = len(_snapshot(**wires)["terms"])
    added = _wire_probes(**wires)

    assert len(added) <= expected, (
        "снимок нёс %d терм(а/ов), а цикл выставил %d проб (%s): рядом с "
        "проводами стоит лишний ключ %s. Имя значения не имеет — это агрегат, "
        "и зелёная лампа рядом с красной гасит именно красную"
        % (expected, len(added), case,
           sorted(set(added) - {k for k in added if _KEY_RE.match(k)})
           or sorted(added)))
    assert len(added) >= expected, (
        "снимок нёс %d терм(а/ов), а цикл выставил %d проб (%s): провод молча "
        "выпал из цикла — «вердикта нет», выданное за «всё хорошо». Пропали: %s"
        % (expected, len(added), case,
           sorted(set(_snapshot(**wires)["terms"])
                  - {_KEY_RE.match(k).group("wire") for k in added
                     if _KEY_RE.match(k)})))
