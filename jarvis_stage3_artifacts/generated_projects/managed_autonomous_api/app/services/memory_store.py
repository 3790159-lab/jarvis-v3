...
class MemoryStore:
    ...
    def add_note(self, note_type: str, text: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not note_type:
            raise ValueError("note_type must not be empty")
        if not text:
            raise ValueError("text must not be empty")
        if len(note_type) > 64:
            raise ValueError("note_type is too long")
        if len(text) > 2000:
    ...
    def validate_store(self) -> Dict[str, Any]:
        ...
        if not isinstance(doc.get("missions"), dict):
            errors.append("missions must be a dict")
        if not isinstance(doc.get("runs"), dict):
            errors.append("runs must be a dict")
        ...
        for note in doc.get("notes", []):
            ...
            if not note.get("type"):
                warnings.append(f"note missing type: {note_id}")
            if not note.get("text"):
                warnings.append(f"note missing text: {note_id}")
